...
SinglePlot::SinglePlot(const QString &pvname, QWidget *parent, const int periodic): QwtPlot(parent),m_pvname(pvname),mpen(Qt::blue),mCount(0),mperiodic(periodic)
{
    ...
    mpicker = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, QwtPicker::PointSelection | QwtPicker::DragSelection, 
...
#if 0
	// Only Static Plot
    m_zoomer[0] = new SinglePlotZoomer( QwtPlot::xBottom, QwtPlot::yLeft, this->canvas());
    ...
    m_zoomer[1] = new SinglePlotZoomer(QwtPlot::xTop, QwtPlot::yRight, this->canvas());
