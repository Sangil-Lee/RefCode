...
{
	d_data->mpCurve->setData(xData, yData);
	replot();
	d_data->mxData = xData;
	d_data->myData = yData;
#if 0
...
void CAWaveformPut::UpdateSlot(const QString nameindex, const int xytype)
{
	...
	d_data->mvecCurve.at(index)->setData(d_data->localxData,d_data->localyData);;
	//d_data->mvecCurve.at(0)->setData(d_data->localxData,d_data->localyData);;
	replot();
}
