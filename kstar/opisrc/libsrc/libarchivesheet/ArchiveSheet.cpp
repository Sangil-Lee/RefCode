...
void ArchiveSheet::showPVList()
{
	...
	int archivekey = m_arch_combo->currentIndex()+1;
	getPVNameList( archivekey, pvname );
}
...
void ArchiveSheet::setupWindow()
{
	...
	{
		setupDialog.hide();
		m_arch_combo->setCurrentIndex(archListCombo->currentIndex());
		getArchiveValues();
    };
};
...
int ArchiveSheet::archiveKey()
{
	return (m_arch_combo->currentIndex()+1);
}
