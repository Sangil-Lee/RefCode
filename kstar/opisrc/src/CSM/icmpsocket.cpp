		...
		vector<string> *replyAddrList, const int timeOut, const int size) throw (Exception)
{
    ...
    while (1) 
	{
		...
		while (((nReadLen = recvfrom (msockFD, buffer, sizeof(buffer), 0, (struct sockaddr*)&from, (socklen_t *)&sl)) 
					< 0) && (EINTR == errno) ) 
		{
			continue;
		};
...
const int ICMPSocket::ping (set<string> *requestAddrList, set<string> *replyAddrList, 
		set<string> &setDiff, const int timeout, const int size)
{
