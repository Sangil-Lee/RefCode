...
int main(int argc, char *argv[])
{
	...
	while (!srcfile.eof ())
	{
		...
		{
			printf("New PVName:%s in source file:%s\n", pvname.c_str(), srcfilename);
			continue;
		};
	...
	if (flist == 1)
	{
		...
		for( str_set_iter = srcfile_set.begin(); str_set_iter != srcfile_set.end(); ++str_set_iter )
		{
			...
			printf("\tNot Fount PV: %s\n", (*str_set_iter).c_str() );
			nofoundcnt++;
		};
		if(nofoundcnt == 0)
			printf("source file exactly exist in destination file.\n");
	};
	if (rlist == 1)
	{
