...
static int  CACommand(string strcmd)
{
	...
	pclose(fp);
	return ((strcmd.compare("ShotNum")== 0||strcmd.compare("ShotType")==0)?atoi(buf):0);
}
...
SessionSummary::SessionSummary():m_con(false),mshotno(0),mbAuto(false)
{
	init();
	getUiObject();
	registerSignal();
...
void SessionSummary::dbSearch()
{
	try {
		...
		{
			strquery= "select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY where shotdate > CURDATE() order by %0 %1";
			query2 << strquery;
			query2.parse();
		...
		else
		{
			...
			{
				strquery="select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY where UNIX_TIMESTAMP(shotdate) > UNIX_TIMESTAMP('%0') and UNIX_TIMESTAMP(shotdate) <= UNIX_TIMESTAMP('%1') order by shotdate %2";
				query2 << strquery;
				query2.parse();
			...
			else
			{
				if(qstrStartVal.isEmpty()==true && qstrEndVal.isEmpty()==true)
				{
					if(mbstart == true )
					{
						strquery= "select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY order by %0 %1 limit 0,30";
						mbstart = false;
					}
					else
					{
						strquery= "select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY order by %0 %1";
					};
					query2 << strquery;
					query2.parse();
					res = query2.use(strPara.c_str(), strOrder.c_str());
				...
				else if(qstrStartVal.isEmpty()==true)
				{
					...
					strquery= "select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY where %0<=%1 order by %0 %2";
					query2 << strquery;
					query2.parse();
				...
				else
				{
					...
					strquery= "select shotnum,shotdate,tfcurrent,plsmacurrent,edensity,etemp,airpressure,echpow,icrhpower,nbi1power,ipwidth,comment,type from SHOTSUMMARY where %0>=%1 and %0<=%2 order by %0 %3";
					query2 << strquery;
					query2.parse();
		...
		while (row = res.fetch_row())
		{
			...
			QString qstrplmacur, qstreden, qstretemp, qstrechp, qstricrfp, qstrnbip, qstripwidth;
...
void SessionSummary::ReadMDSData(int shotno, bool update)
{
	...
	for (int i = 0;i<8;i++)
	{
		...
		else if( strPara[i].compare("NB1_PB1")==0)	//2011
		{
			// 2011, After Calibration, will be fixed.
			mxVal= 0.0;
		}
	...
	qDebug("ICRF-Power:%f,NBI-Power:%f",dMxval[6], dMxval[7]);
	...
	try
	{
		...
		SHOTSUMMARY shotsummaryinsert(shotno,shotdate,dMxval[0],dMxval[1],dMxval[2],0.0,dMxval[3],dMxval[5],dMxval[6],dMxval[7],timeWidth,timeWidth100k,"", strShotType, "2010C03");
