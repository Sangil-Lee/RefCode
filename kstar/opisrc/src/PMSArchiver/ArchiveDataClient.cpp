                                  ...
                                  value_callback callback, void *callback_arg, double interval, int & loopcnt, int & arrycnt)
{
    ...
    for (n=0; n<channel_count; ++n)
    {
        ...
        if (log_fault())
            return false;
        if (!decode_channel(channel_result, n, callback, callback_arg, arrycnt))
            return false;
    }
    xmlrpc_DECREF(result);
    return true;
									   ...
									   int & arrycnt)
{
    ...
    return decode_data(name, type, count, data, ctrlinfo,
                       n, callback, callback_arg, arrycnt);
}
...
void ArchiveDataClient::epicsTime2pieces(const epicsTime &t, xmlrpc_int32 &secs, xmlrpc_int32 &nano)
{   
...
void ArchiveDataClient::pieces2epicsTime(xmlrpc_int32 secs, xmlrpc_int32 nano, epicsTime &t) 
