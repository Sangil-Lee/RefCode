	...
	else
	{
		...
		if(pSummary->GetAuto() == true && pdata->value == (long)1)
		{
			...
			if(hostname.compare("opi16")==0)
			{
				...
				qDebug("Update--MDS_SHOT_NUMBER:%d",updateshot);
				pSummary->SetShotType(CACommand("ShotType"));
				pSummary->ReadMDSData(updateshot);
				pSummary->LastShotNumber(updateshot);
				//CACommand("ResetShotSummary");

				//string strCmd = "caput DEV02_DAQDATA_SCREENSHOT 0";
				//system(strCmd.c_str());
			}
			else
			{
...
#if 0
	SEVCHK(ca_create_channel("DEV02_DAQDATA_SCREENSHOT",connectionCallback,
				&mynode,10,(oldChannelNotify**)&mynode.summary_chid), "ca_create_channel");
#else
	SEVCHK(ca_create_channel("PCS_LS_STATUS",connectionCallback,
				&mynode,10,(oldChannelNotify**)&mynode.summary_chid), "ca_create_channel");
#endif
