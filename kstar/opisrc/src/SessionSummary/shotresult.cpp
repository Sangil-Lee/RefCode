...
ShotResult::ShotResult():m_con(false),mfilepath("/home/kstar/screenshot/"),getProc(),putProc(),mauto(false)
{
	init();
	getUiObject();
	registerSignal();
...
{
	readImage(milbPCS, QString("pcsdata_shot%1.png").arg(shotno));
	readImage(milbDAQ, QString("daqdata_shot%1.png").arg(shotno));
}
...
{
	qDebug("MakeImage****");
	QString filename = QString("pcsdata_shot%1.png").arg(shotno); 
	...
	filename = QString("daqdata_shot%1.png").arg(shotno); 
	...
	{
		filename = QString("shotresult_%1.png").arg(shotno); 
...
void ShotResult::makeImage()
{
	...
	if(m_pParent->GetHostname().compare("opi16")==0) 
	{
		QString filename = QString("shotresult_%1.png").arg(shotno); 
		qDebug("exportImage:%s",filename.toStdString().c_str());
		exportImage(filename);
	};
#endif
}
void ShotResult::exportImage(QString filename)
{
	...
	QString fullpath =QString("/home/kstar/shotresult/%1").arg(filename);
...
int ShotResult::downloadFile(const QString filename)
{
	...
	QString cmd="ncftpget -u kstar -p kstar2007 172.17.100.214 /home/kstar/screenshot/ /home/kstar/screenshot/" + filename;
...
{
#if 1
	//QString cmd="ncftpput -u kstar -p kstar2007 172.17.100.204 /usr/local/tomcat5/webapps/kstarweb/intranet/shotresult/ ../shotresult/" + filename;
	QString cmd="ncftpput -u kstar -p kstar2007 172.17.100.204 /home/kstar/shotresult/ /home/kstar/shotresult/" + filename;
	FILE *fp = popen(cmd.toStdString().c_str(), "r");
	pclose(fp);
#else
...
int ShotResult::readImage(QLabel *imageLabel, QString filename)
{
	...
	{
		QString strmsg= QString("Found file %1").arg(fullfilepath);
		qDebug("%s",strmsg.toStdString().c_str());
