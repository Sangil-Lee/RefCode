...
int ca_Connect( char *strPVname, chid &chidChannelToPV)
{
	...
	iCAStatus = ca_pend_io( PEND_TIME );
	if( iCAStatus != ECA_NORMAL){
		return iCAStatus;
	}
	return iCAStatus;
...
int ca_PutST_QStr(ST_PV_NODE &myPV, QString oQString)
{
	int iCAStatus = ECA_NORMAL;
	unsigned long nelm;

	if( (iCAStatus = ca_write_access( myPV.cid) ) != ECA_NORMAL ){
		return ECA_NOWTACCESS;
	}

	nelm = ca_element_count( myPV.cid);
	long lTotalLength = dbr_size_n( dbf_type_to_DBR( ca_field_type( myPV.cid) ), nelm );
	void * AvPutData = new char [ lTotalLength];
	if( AvPutData == NULL){ 
		iCAStatus = ECA_ALLOCMEM;
		return iCAStatus;
...
//				break;
	...
	iCAStatus = ca_array_put( dbf_type_to_DBR( ca_field_type( myPV.cid) ), 
			nelm, myPV.cid, AvPutData);
	if( iCAStatus != ECA_NORMAL){
		delete [] ( char *) AvPutData;
		return iCAStatus;
	}
	...
	myPV.status_flag |= PV_STATUS_TOUCH;
...
int ca_PutST_Double(ST_PV_NODE &myPV, double dval)
{
	...
	if( ca_field_type(myPV.cid) != DBF_DOUBLE ) {
		iCAStatus = ECA_BADTYPE;
		return iCAStatus;
	}
	long lTotalLength = dbr_size_n( dbf_type_to_DBR( ca_field_type( myPV.cid) ), nelm );
...
int ca_GetST_QStr(ST_PV_NODE &myPV, QString &qstrVal)
{
	...
	if( (iCAStatus=ca_read_access( myPV.cid)) != ECA_NORMAL ){
		return ECA_NORDACCESS;
	}
	nelm = ca_element_count( myPV.cid);

	long lTotalLength = dbr_size_n( dbf_type_to_DBR_TIME( ca_field_type( myPV.cid) ),  nelm);
	void * AvGetData = new char [ lTotalLength];
	if( AvGetData == NULL){ 
		iCAStatus = ECA_ALLOCMEM;
		return iCAStatus;
	}
	iCAStatus = ca_array_get( dbf_type_to_DBR_TIME( ca_field_type( myPV.cid) ),
			nelm, myPV.cid, AvGetData);
	if( iCAStatus != ECA_NORMAL){
		delete [] ( char *) AvGetData;
		return iCAStatus;
	}
