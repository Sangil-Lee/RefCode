...
void buffers(float* M_A, float* M_B, float* M_C, size_t arr_size)
{
  ...
  mem_A = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
		  sizeof(float)*arr_size, M_A, &err);
  CHECK_ERROR(err);

  mem_B = clCreateBuffer(context, CL_MEM_READ_ONLY,
		  sizeof(float)*arr_size, NULL, &err);
  CHECK_ERROR(err);
  ...
  // Write to Buffers
  /* TODO */
  err = clEnqueueWriteBuffer(cmd_queue, mem_B, CL_TRUE, 0, sizeof(float)*arr_size, M_B, 0, NULL, NULL);
  CHECK_ERROR(err);
  ...
  err = clEnqueueReadBuffer(cmd_queue, mem_C, CL_TRUE, 0, sizeof(float)*arr_size, M_C, 0, NULL, NULL);
