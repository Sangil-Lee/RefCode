...
void CL_CALLBACK write_completed(cl_event ev, cl_int status, void *data)
{
	printf("clEnquueWirteBuffer is completed\n");
}

void CL_CALLBACK read_completed(cl_event ev, cl_int status, void *data)
{
	printf("clEnquueReadBuffer is completed\n");
}

void CL_CALLBACK kernel_completed(cl_event ev, cl_int status, void *data)
{
	printf("clEnquueNDRangekernel is completed\n");
}
...
void buffers(float* M_A, float* M_B, float* M_C, size_t arr_size)
{
  ...
  err = clEnqueueWriteBuffer(cmd_queue,
                             ...
                             sizeof(float) * arr_size,
                             M_B,
                             0, NULL,&ev_write_buffer);
  CHECK_ERROR(err);
  err = clSetEventCallback(ev_write_buffer, CL_COMPLETE, &write_completed, NULL);
  ...
  err = clEnqueueNDRangeKernel(cmd_queue,
                               ...
                               0, NULL, &ev_kernel);
  CHECK_ERROR(err);
  err = clSetEventCallback(ev_kernel, CL_COMPLETE, &kernel_completed, NULL);
  ...
  err = clEnqueueReadBuffer(cmd_queue,
                            ...
                            sizeof(float) * arr_size,
                            M_C,
                            0, NULL, &ev_read_buffer);
  CHECK_ERROR(err);
  err = clSetEventCallback(ev_read_buffer, CL_COMPLETE, &read_completed, NULL);
