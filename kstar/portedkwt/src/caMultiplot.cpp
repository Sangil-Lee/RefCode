...
void CAMultiplot::initMultiPlot()
{
	...
	d_data->mgrid->setMajorPen(QPen(Qt::black, 0, Qt::DashLine));
	d_data->mgrid->setMinorPen(QPen(Qt::gray , 0, Qt::DotLine));
...
void CAMultiplot::Run()
{
    ...
    d_data->mpicker = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, QwtPicker::RectRubberBand, 
	...
	for(size_t index = 0; index<pvcnt; index++ )
	{
		...
		//d_data->mpCurve[index]->setRawSamples(d_data->mtimeline, d_data->mdata[index], PrivateData::PLOT_SIZE);
...
{ 
	...
	d_data->mpCurve[index]->setRawSamples(d_data->mtimeline, d_data->mdata[index], xscale);
#endif
...
void CAMultiplot::setMajorlinecolor(const QColor &color)
{
	...
	d_data->mgrid->setMajorPen(QPen(color, 0, Qt::DashLine));
	plotupdate();
}
...
{
	d_data->mgrid->setMinorPen(QPen(color , 0, Qt::DotLine));
