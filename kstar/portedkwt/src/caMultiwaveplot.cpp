...
void CAMultiwaveplot::Run()
{
    ...
    d_data->mpicker = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, 
        QwtPicker::RectRubberBand, QwtPicker::ActiveOnly, canvas());
...
void CAMultiwaveplot::SetData()
{
	...
	for(size_t index = 0; index<pvcnt; index++ )
	{
		...
		d_data->mpCurve[index]->setRawSamples(d_data->mtimeline, d_data->mbuffer[index], d_data->mreqelement*getBufferPage());
