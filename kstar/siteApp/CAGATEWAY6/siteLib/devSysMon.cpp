...
public:
	...
	enum USERPROC {
		TMS=1, VMS=TMS*2, ECH=VMS*2, SCS=ECH*2, FUEL=SCS*2, GCDS=FUEL*2, ICRH=GCDS*2, CSM=ICRH*2, MPS=CSM*2
	};
...
unsigned int SysMonInfo::getUserProcessStatus()
{
	...
	for(svclistiter=m_userproclist.begin(); svclistiter!=m_userproclist.end();++svclistiter)
	{
		...
		else if(strprocess.compare("SCS") == 0)
		{
			if (checkProcess("SCS") == 0)
				procstatus &= ~SCS;
			else
				procstatus |= SCS;
			continue;
		...
		else if(strprocess.compare("CSM") == 0)
		{
			if (checkProcess("CSM") == 0)
				procstatus &= ~CSM;
			else
				procstatus |= CSM;
			continue;
		}
		else if(strprocess.compare("MPS") == 0)
		{
			if (checkProcess("MPS") == 0)
				procstatus &= ~MPS;
			else
				procstatus |= MPS;
			continue;
...
unsigned long myIpAddr(const char* ifname)
{
  int s;
  unsigned long r;
  struct ifreq ifr;
...
static long read_uptime_IP(void *pIn)
{
  ...
  /*     long status; */
    ...
    /*If return was succesful then set undefined false*/
    /* sprintf(pStringIn->val,"ala %d",get_uptime()); */
	if(strcmp(pvmeio->parm,"eth1")==0)
		iPAddr=myIpAddr("eth1");
	else if(strcmp(pvmeio->parm,"eth2")==0)
		iPAddr=myIpAddr("eth2");
	else if(strcmp(pvmeio->parm,"eth3")==0)
		iPAddr=myIpAddr("eth3");
	else
		iPAddr=myIpAddr("eth0");
...
static int executeCommand( const string command )
{
	...
	if (strCmd.compare("OPIUPDATE") == 0)
	{
		...
		fp = popen ("opiupdate &", "r");
	}
	else if (strCmd.compare("OPIINSTALL") == 0 )
	{
		if(strstr(host.c_str(), "opi") == NULL) goto BREAK; 
		fp = popen ("opiinstall &", "r");
	}
