...
static int executeCommand( const string command )
{
	...
	{
		if(host.compare("data3") != 0) goto BREAK;
		chdir("/home/efit/EFIT_RUN/EFITDATA");
		string efitCmd;
		if (paramSize == 1)
		{
			efitCmd = string("/home/efit/EFIT_RUN/EFITDATA/runefit ")+vecParam.at(0)+string(" &");
		}
		else if (paramSize == 2)
		{
			efitCmd = string("/home/efit/EFIT_RUN/EFITDATA/runefit ")+vecParam.at(0)+string(" ")+vecParam.at(1)+string(" &");
		}
		else if (paramSize == 3)
		{
			efitCmd = string("/home/efit/EFIT_RUN/EFITDATA/runefit ")+vecParam.at(0)+string(" ")+vecParam.at(1)+string(" ")+vecParam.at(2)+string(" &");
		}
		else if (paramSize == 4)
		{
			int shotno = atoi(vecParam.at(0).c_str());
			shotno = shotno-1;
			char strshot[12];
			memset(strshot,0, sizeof(strshot));
			sprintf(strshot, "%d", shotno);
			string strshotno = string(strshot);

			//efitCmd = string("/home/efit/EFIT_RUN/EFITDATA/runefit ")+vecParam.at(0)+string(" ")+vecParam.at(1)+string(" ")+vecParam.at(2)+string(" ")+vecParam.at(3)+string(" &");
			efitCmd = string("/home/efit/EFIT_RUN/EFITDATA/runefit ")+strshotno+string(" ")+vecParam.at(1)+string(" ")+vecParam.at(2)+string(" ")+vecParam.at(3)+string(" &");

			printf("%s\n", efitCmd.c_str());
			fp = popen (efitCmd.c_str(), "r");

			chdir("/home/ra9105/make_rtefit/");
			string efitMovie = string("./MDS_rtEFIT_2dplot.py ") + strshotno + string(" &");
			printf("%s\n", efitMovie.c_str());
			system(efitMovie.c_str());
			//fp = popen (efitMovie.c_str(), "r");
		...
		string efitMovie = string("/home/ra9105/make_rtefit/MDS_rtEFIT_2dplot.py ") + vecParam.at(0)+string(" &");
		printf("%s\n", efitMovie.c_str());
		fp = popen (efitMovie.c_str(), "r");
