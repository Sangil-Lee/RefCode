...
/* don't use "epicsPrintf" i want get print out immediately             2009. 10. 17*/
...
/*	printf("\n>>> %s: clock divider is %d\n", pSTDdev->taskName, pSTDdev->clockDivider); */
	
	sprintf(strCmd, "acqcmd -b %d setExternalClock DI0 %d", pAcq196->slot, pAcq196->clockDivider);
/*	sprintf(strCmd, "acqcmd -s %d set.ext_clk DI0 %d", pAcq196->slot, pAcq196->clockDivider); */
	pfp=popen( strCmd, "r");
	if(!pfp) {
		printf("\n>>> %s: acqcmd -b %d setExternalClock DI0 %d", pSTDdev->taskName, pAcq196->slot, pAcq196->clockDivider);
		return WR_ERROR;
	}	while(fgets(buff, 128, pfp) != NULL ) { 
#if PRINT_ACQCMD
		printf("\n>>> %s: %s: %s", pSTDdev->taskName, strCmd, buff); 
