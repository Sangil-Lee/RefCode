...
int gain4Desc, gain12Desc, gain20Desc, gain27Desc; /* Gain  */
int offset4Desc, offset12Desc, offset20Desc, offset27Desc; /* Offset value */
...
/* Data Channelization Part  */
	...
	int int32TempBuf=0;
/*	int int32TempBuf2=0;  */
	int int32TempBuf96[64]; /* All Channel 96 Channel  */
...
/* Add TG.Lee MDSplus Time Array Infomation Variable */
	...
	float gain4, gain12, gain20, gain27;
	float offset4, offset12, offset20, offset27;
...
/* TG.Lee ******** Data Channelization  Start     */
	...
	{
		readStatus = fread( &int32TempBuf96, 4, 64, fpRaw );
		if(readStatus != 0){
			nCnt++;
			if( isFirstNode ) {
...
/*	memset(data, 0, sizeof(float)*g_sampleDataCnt);  */
	...
	gain4Desc = descr(&dtypeFloat, &gain4, &null);
	gain12Desc = descr(&dtypeFloat, &gain12, &null);
	gain20Desc = descr(&dtypeFloat, &gain20, &null);
	gain27Desc = descr(&dtypeFloat, &gain27, &null);

	offset4Desc = descr(&dtypeFloat, &offset4, &null);
	offset12Desc = descr(&dtypeFloat, &offset12, &null);
	offset20Desc = descr(&dtypeFloat, &offset20, &null);
	offset27Desc = descr(&dtypeFloat, &offset27, &null);
...
*/
		if(readStatus !=0){
			if( 32 <= i && i < 64){
				...
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS($*$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))",&range10Desc, &dataDesc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			} else if(i==3){
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))", &dataDesc,&range10Desc, &offset4Desc, &gain4Desc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			} else if(i==11){
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))", &dataDesc,&range10Desc, &offset12Desc, &gain12Desc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			} else if(i==19){
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))",  &dataDesc, &range10Desc, &offset20Desc, &gain20Desc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			} else if(i==26){
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))", &dataDesc, &range10Desc, &offset27Desc, &gain27Desc,  &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			} else {
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
				status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS($*$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))", &range10Desc, &dataDesc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			}		
...
int dataChannelization()
{
	...
	unsigned int i, j;
	int dma_cnt, dmaBlock_size;
	int int32TempBuf=0, int32TempData96[64];
...
*/
	...
	{
		fread( &int32TempData96, 4, 64, fpRaw ); 
		for( j=0; j< used_channel; j++) 
		{
/*			fread( &int32TempBuf, 4, 1, fpRaw );  */
#if 0
