...
/************ MDSplus Server Information File ****************************/
	if( (fpConfig = fopen("/root/ioc/config/mdsplus.cfg", "r") ) == NULL )
	{	
...
/*	memset(data, 0, sizeof(float)*g_sampleDataCnt);  */
	...
	/*	startTime = 0;   	Data Zero Start Time */
		...
		endTime = numSamples + startTime;  /* -1 sec */
	}
...
*/
			if(readStatus !=0){
			   if( 32 <= i && i < 64){
/*				dataDesc = descr(&dtypeFloat, data, &sampleReCount, &null); dtypeFloat is  Data Trans Error int -> float  */
				...
				dataDesc = descr(&dtypeLong, data, &numSamples, &null);     
        	        	status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS($*$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))", &range5Desc, &dataDesc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);
			   }		
			   if ( !statusOk(status) )
			   {
