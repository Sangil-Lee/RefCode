...
STATUS enableStrobeReception (int dpio2DeviceNumber, UINT32 strobeFrequency)
{
  ...
  status = dpio2Ioctl (dpio2DeviceNumber, DPIO2_CMD_CLOCKING_ON_BOTH_STROBE_EDGES_SELECT, FALSE);
  if (status != OK) {
    printf("Failed to set as strobe edges disable clocking \n");
    return (ERROR);
  }
...
{
	status = dpio2CacheInvalidate (pPageInfo_in);
	if (status != OK) {
    	printf ("Failed to configure DMA chain\n");
    	return (-1);
  	}
