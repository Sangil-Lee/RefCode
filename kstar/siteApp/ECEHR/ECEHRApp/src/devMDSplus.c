...
/* TG.Lee skip data 61(internal clock) or 47(external clock) points for each channel.   */
#if 1 
        for (i=1; i < 62; i++)
        {
                readStatus = fread( &int32TempBuf96, 4, 96, fpRaw );
        }
	sampleCnt = sampleCnt - 61;
#endif
/* or fseek function used ? 4*32*47 = 6016  or 4*32*61 = 7808  */
#if 0
        fseek(fpRaw, 7808, SEEK_SET);
	sampleCnt = sampleCnt - 61;
#endif
...
/*	memset(data, 0, sizeof(float)*g_sampleDataCnt);  */
	...
	/*	startTime = 0;   	Data Zero Start Time */
		startTime = fT0;  	/* Data Zero Start Time */
		endTime = numSamples -1;		 /* endTime = sampleReCount;  Last data Zero Over Count.  */
		/*fprintf(stdout,"\nMDSput Data Blip Time = %f  .... OK??\n",blip);  Test */ 
	} else { 
		startTime = (fT0 - blip);  /* "-1" Value is Data Start Time ..... */
					/* endTime = (sampleReCount-1) + startTime;   -1 sec */
		endTime = numSamples -1;  /* -1 sec */
	}
...
/* move before 1msec delay error 	status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW($,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))",&range10Desc, &dataDesc,&offsetDesc, &gainDesc, &startDesc, &endDesc, &rateDesc, &rateDesc, &null);  TG */
			status = MdsPut(channelTagName[i], "BUILD_SIGNAL(BUILD_WITH_UNITS(($*$-$)/$,\"V\"),,MAKE_DIM(MAKE_WINDOW(0,$,$),MAKE_SLOPE(MAKE_WITH_UNITS($,\"s\"))))",&range10Desc, &dataDesc,&offsetDesc, &gainDesc, &endDesc, &startDesc, &rateDesc, &null);
			if ( !statusOk(status) )
			{
...
#endif
/* Close tree we need not this function.
	if(mode == OPMODE_REMOTE){
