...
void armingDeviceParams (ST_STD_device *pSTDdev)
{
	...
	for(i=0;i<chanNum;i++) {
		...
		pACQ196->ST_Ch[i].teValue		= 0;
		pACQ196->ST_Ch[i].teOffset		= 0;
...
void processECEposition (ST_STD_device* pSTDdev)
{
	...
	kLog (K_MON, "[processECEposition] %s : Start Calc ECE Channel Position about TF(%f)\n", pSTDdev->taskName, pACQ196->currentTF);
	...
	measuredTF = pACQ196->currentTF;    	// currentTF(kA) * 1000 = A,  default 19640 A
        chanNum = ellCount(pSTDdev->pList_Channel);
				...
				if(fpdata == NULL){
					...
					if( pACQ196->fileHandling == 1){
						...
						{ 
							...
							if(pACQ196->totalRead == 0){
								pACQ196->ST_Ch[j].teOffset = pACQ196->ST_Ch[j].euValue;
							}
							pACQ196->ST_Ch[j].teValue = (float64)((pACQ196->ST_Ch[j].euValue - pACQ196->ST_Ch[j].teOffset) * pACQ196->ST_Ch[j].calFactor);
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	...
	pACQ196->currentTF			= 19640;                // 19.64kA -> 1.964 T
