...
static long devWaveNIDAQmx_read_wave(waveformRecord *precord)
{
	...
	switch (pSTdpvt->ind) {
		case WAVE_RAW_VALUE :
			...
			precord->bptr = pNIDAQmxAICh->fScaleReadChannel;
			if(sampleRate > 0) precord->nord = sampleRate;
			kLog (K_DATA, "[devWaveNIDAQmx_read_wave.WAVE_RAW_VALUE] %s:%d raw value: %f \n",
					pSTDdev->taskName, chId, pNIDAQmxAICh->fScaleReadChannel[0]);
			break;

		case WAVE_VALUE :
			if(sampleRate > 0) precord->nelm = sampleRate;
			precord->bptr = pNIDAQmxAICh->calcDensity;
			if(sampleRate > 0) precord->nord = sampleRate;
			kLog (K_DATA, "[devWaveNIDAQmx_read_wave.WAVE_VALUE] %s:%d value: %f\n", pSTDdev->taskName, chId, pNIDAQmxAICh->fScaleReadChannel[0]);
			break;
