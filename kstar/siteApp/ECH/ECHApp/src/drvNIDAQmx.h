...
#define NIDAQ_AO_CH_STR				"AO"	        // AO Data
...
// output PV
#define AO_DAQ_PATTERN_VALUE_STR		"pattern_value"
#define AO_DAQ_PATTERN_POWER_STR        	"pattern_power"
#define AO_DAQ_PATTERN_TIME_STR	        	"pattern_time"
#define AO_DAQ_VALUE_STR			"output_value"
...
#define BO_SELECT_AO_MODE_STR			"select_aomode"
#define BO_MAKE_PATTERN_STR			"make_pattern"
#define BO_STORE_PATTERN_STR			"store_pattern"
#define BO_DOING_PROGRESS_STR			"doing_progress"
...
#define AO_DAQ_PATTERN_VALUE                    1119
#define AO_DAQ_PATTERN_POWER                    1120
#define AO_DAQ_PATTERN_TIME 			1121
#define AO_DAQ_VALUE 	            	    	1122
...
#define BO_SELECT_AO_MODE			3006
#define BO_MAKE_PATTERN				3007
#define BO_STORE_PATTERN			3008
#define BO_DOING_PROGRESS			3009

// PVs
#define PV_LOCAL_SHOT_NUMBER_STR 	"ECH_AO_SHOT_NUMBER"
#define PV_SYS_ARMING_STR 		"ECH_AO_SYS_ARMING"
#define PV_SYS_RUN_STR			"ECH_AO_SYS_RUN"
#define PV_LTU_TIG_T0_STR		"FUEL_LTU_P6_SEC0_T0"
#define PV_START_TIG_T0_STR		"ECH_AO_START_T0_SEC1"
#define PV_STOP_TIG_T1_STR		"ECH_AO_STOP_T1_SEC1"
#define PV_RESET_STR 			"ECH_AO_RESET"

#define PV_CALC_RUN_STR 		"ECH_AO_CALC_RUN"
#define PV_TREND_RUN_STR 		"ECH_AO_TREND_RUN"
#define PV_DAQ_STOP_STR 		"ECH_AO_DAQ_STOP"

// PVs for ECH param
#define PV_ECH_AO_OUTPUT_VALUE_PWR_CH0_STR "ECH_AO_OUTPUT_VALUE_PWR_CH0"
#define PV_ECH_AO_OUTPUT_VALUE_CH0_STR 	   "ECH_AO_OUTPUT_VALUE_CH0"
#define PV_ECH_AO_OUTPUT_VALUE_CH1_STR 	   "ECH_AO_OUTPUT_VALUE_CH1"
#define PV_ECH_AO_OUTPUT_VALUE_CH2_STR 	   "ECH_AO_OUTPUT_VALUE_CH2"
#define PV_ECH_AO_OUTPUT_VALUE_CH3_STR 	   "ECH_AO_OUTPUT_VALUE_CH3"

#define PV_ECH_POL_PH1_STR		"ECH_AO_POL_ANG_PHI1_MDS"
#define PV_ECH_POL_PH2_STR		"ECH_AO_POL_ANG_PHI2_MDS"
#define PV_ECH_ANT_POL_STR		"ECH_AO_ANT_ANG_POL_MDS"
#define PV_ECH_ANT_TOR_STR		"ECH_AO_ANT_ANG_TOR_MDS"

// initial value
#define INIT_CHECK_INTERVAL		1       /* Interval for Check the Configuration in run as second */
#define INIT_MAX_CHAN_NUM		16	/* PCIe-6259 diff-16, single-32, 32ch - 6254, 8ch-DAQ Max Channel number of DAQ unit ,but mmwi used from 4ch to 2ch because Not Ready FIR , SCXI for STRAIN = 8x4=32  */	
#define INIT_SAMPLE_RATE		10000		// Hz
#define INIT_SAMPLE_RATE_LIMIT		1000000		// Hz, for NI6220 Device. It's depends on device and channel 
#define INIT_MAX_CHAN_SAMPLES  		1000000  	/* first value : 100,000 but PXI-DAQ Max Rate : 500,000 Hz. so number of sampling data per channel, 6259:1MHz  */
...
typedef struct {
	...
	int		channelId;		// infomation about number of channel
...
typedef struct {
	...
	float64		integral_time;			// ao : PV.TIME
	float64		integral_cnt;			// integral_time * sample_rate
	float64		euAoValue;			// ao : PV.VAL
	float64		power;				// ao : PV.POWER

	float64		voltOut;			// volt output control on real-time
	float64		powerOut;			// power output control on real-time
	// multi wave pattern information for prepare generate multi wave pattern 20120202
	float64		wavePatternPower[80];		// multi wave pattern power- 80 points = volt x power scale
	float64		wavePatternValue[80];		// multi wave pattern value- 80 points
	float64		wavePatternTime[80];		// multi wave pattern time- 80 points
} ST_NIDAQmx_AOchannel;

typedef struct {
	TaskHandle	taskHandle;
	char		triggerSource[30];		// /Dev1/PFI0
	char		clockSource[30];		// /Dev1/PFI12
	char		sigType[30];			// ai, ao, port, line ?

	// SFW
	int		smp_mode_select;		// Select on OPI(PV) AO device DAQmx_Val_FiniteSamps or DAQmx_Val_ContSamps
	int32		smp_mode; 			// To save info about DAQmx_Val_FiniteSamps or DAQmx_Val_ContSamps
	...
	// Event
	int32		totalRead;
	int32		totalWrite;			// write data counts per each channel 
	...
	ELLLIST		*pchannelConfig;		// for configurate the channel node by node

	ST_NIDAQmx_AIchannel ST_Ch[INIT_MAX_CHAN_NUM]; //Remove ????? already make channelConfig
	short		readArray[INIT_TOT_EVENT_SAMPLES];		// 18bit for change tye from short to int32 buffer for raw sampling data
...
extern float64 getTotSampsChan (ST_NIDAQmx *pNIDAQmx);
...
extern int drvNIDAQmx_AO_DAQ_VALUE (ST_STD_device *pSTDdev);
extern int resetDevice (ST_STD_device *pSTDdev);
extern int processDensity (ST_NIDAQmx *pNIDAQmx, int chNo);
...
extern int proc_sendAOData2Tree(ST_STD_device *pSTDdev);
extern void make_pattern_dataToFile(ST_STD_device *pSTDdev);
extern void AO_Make_Status_Progress(ST_STD_device *pSTDdev);
