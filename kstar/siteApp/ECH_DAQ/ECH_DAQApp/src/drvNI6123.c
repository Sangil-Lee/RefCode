...
{ 
	return (pNI6123->sample_rate / 10); 
//	return (pNI6123->sample_rate); // Test every event per 10msc 
}
	...
	while (TRUE) {
		...
		if (pSTDdev->StatusDev & TASK_STANDBY) {
			...
			/*	processAutoTrendRun (pSTDdev);   */
				...
				/*      This Function have to perform after  Channel created.. */
        		       // Not used this time	NIERROR(DAQmxGetSampClkMaxRate(pNI6123->taskHandle, &(pNI6123->sample_rateLimit)));
...
int create_NI6123_taskConfig(ST_STD_device *pSTDdev)
{
	...
	for (i=0;i<INIT_MAX_CHAN_NUM;i++) {
		...
		if (NULL != (pSTDCh = get_STCh_from_STDev_chID (pSTDdev,i))) {
			...
			if (0 == strcmp (NBI_INPUT_TYPE_DT_STR, pSTDCh->strArg0)) {
				...
				pNI6123->ST_Ch[i].inputType = MDS_PARAM_PUT;
			}
			else {
	...
	pNI6123->terminal_config		= DAQmx_Val_RSE;	// DAQmx_Val_Diff(6123), DAQmx_Val_NRSE NI-6254&NI6220(DAQmx_Val_RSE)
	pNI6123->minVal				= INIT_LOW_LIMIT;
	pNI6123->maxVal				= INIT_HIGH_LIMIT;
	pNI6123->sample_rate			= INIT_SAMPLE_RATE;
	pNI6123->sample_rateLimit		= INIT_SAMPLE_RATE_LIMIT;
			...
			for(i=0;i<INIT_MAX_CHAN_NUM;i++) {
				if(pNI6123->ST_Ch[i].inputType == DENSITY_CALC) {
					...
					epicsPrintf(" Ch No:%d, Tag Name : %s, euValue : %f \n",
						i, pNI6123->ST_Ch[i].strTagName,pNI6123->ST_Ch[i].euValue);
					epicsPrintf("\n");
