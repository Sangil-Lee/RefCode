...
static void devNIDAQ_BO_SELECT_STORE(ST_execParam *pParam)
{
	...
	kLog (K_TRACE, "[devNIDAQ_BO_SELECT_STORE] %s : ch(%d) Store Select value(%f)\n",
			pSTDdev->taskName, chId, pParam->setValue);

	pNIDAQ->ST_Ch[chId].saveToMds = pParam->setValue;
	...
	if(precord->pact == FALSE) {	
		...
		switch(pSTdpvt->ind) {
			...
			case BO_STORE_TO_MDS:
				qData.pFunc = devNIDAQ_BO_SELECT_STORE;
				break;
			default: 
				break;
		}
