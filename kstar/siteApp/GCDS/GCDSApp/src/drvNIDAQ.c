...
int isSamplingStop (ST_STD_device *pSTDdev)
{
	...
	return ( isCalcRunMode() ? (pNIDAQ->totalRead >= getTotSampsChan(pNIDAQ)) : FALSE );
}
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	...
	kLog (K_MON, "[EveryNCallback] %s : type(%d) nSamples(%d) rate(%.f) status(%s) total(%d) mode(%d) shot(%lu/%lu)\n",
			...
			pNIDAQ->totalRead, isCalcRunMode(), get_master()->ST_Base.shotNumber, pSTDdev->ST_Base_fetch.shotNumber);
...
*/
	...
	for (i=0;i<totSampsAllChan;i++) {
		pNIDAQ->readArray[i] = GET_RAW14_VALUE(pNIDAQ->fScaleReadArray[i]);
		...
		if (pSTDdev->StatusDev & TASK_STANDBY) {
			...
			/*	processAutoTrendRun (pSTDdev);   */
				...
				if(pMaster->n8MdsPutFlag == 0) {
					...
					stop = wf_getCurrentUsec();
					kLog (K_INFO,"[threadFunc_NIDAQ_DAQ - write to mdsplus DB Mode (%d) ] : measured function process time sec. is %lf msec\n",
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	...
	pSTDdev->ST_Func._CREATE_LOCAL_TREE	= func_NIDAQ_CREATE_LOCAL_SHOT;
