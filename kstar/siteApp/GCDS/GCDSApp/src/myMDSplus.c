...
int proc_sendData2Tree (ST_STD_device *pSTDdev)
{
	...
	float64 fVoltRange		= EU_GRADIENT14;
...
// for density data
	...
	for (i = 0; i<chanNum; i++) {
		if (i !=1){
			// GCDS device channel #1 (0~start) have much noise, so I will do not store in this channel. 2012.08.29
			...
			if(pNIDAQ->ST_Ch[i].inputType == MDS_PARAM_PUT) {
				...
				if(i==0){
					DBproc_get (PV_ECH_POL_PH1_STR, strBuf);
					sscanf (strBuf, "%f", &dataBuf);
					sprintf(tdiBuffer, "FS_FLOAT(%f)", dataBuf);
					status = MdsPut("\\ECH_POL_PH1", tdiBuffer, &_null);
					kLog (K_MON, "[proc_sendData2Tree] MDS param Put for ECH %s : ECH POL PH1 Value :  %f \n", pSTDdev->taskName, dataBuf);

					DBproc_get (PV_ECH_POL_PH2_STR, strBuf);
					sscanf (strBuf, "%f", &dataBuf);
					sprintf(tdiBuffer, "FS_FLOAT(%f)", dataBuf);
					status = MdsPut("\\ECH_POL_PH2", tdiBuffer, &_null);
					kLog (K_MON, "[proc_sendData2Tree] MDS param Put for ECH %s : ECH POL PH2 Value :  %f \n", pSTDdev->taskName, dataBuf);

					DBproc_get (PV_ECH_ANT_POL_STR, strBuf);
					sscanf (strBuf, "%f", &dataBuf);
					sprintf(tdiBuffer, "FS_FLOAT(%f)", dataBuf);
					status = MdsPut("\\ECH_ANT_POL", tdiBuffer, &_null);
					kLog (K_MON, "[proc_sendData2Tree] MDS param Put for ECH %s : ECH ANT POL Value :  %f \n", pSTDdev->taskName, dataBuf);

					DBproc_get (PV_ECH_ANT_TOR_STR, strBuf);
					sscanf (strBuf, "%f", &dataBuf);
					sprintf(tdiBuffer, "FS_FLOAT(%f)", dataBuf);
					status = MdsPut("\\ECH_ANT_TOR", tdiBuffer, &_null);
					kLog (K_MON, "[proc_sendData2Tree] MDS param Put for ECH %s : ECH ANT TOR Value :  %f \n", pSTDdev->taskName, dataBuf);
				}
			}
#endif	
		}
