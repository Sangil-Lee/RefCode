...
/* Data Channelization Part  */
	...
	int int32TempBuf=0;
/*	int int32TempBuf2=0;  */
	int int32TempBuf96[32]; /* First 96ch but I will use 32 ch.   All Channel 96 Channel  */
...
/*	we need not this func	else if( strcmp(buf, "REMOTE_TREE2") == 0) {
...
#endif
/* TG.Lee ******** Data Channelization  Start   I will used 32 channel so raw data file counts is 32.  */
	nCnt=0;
	fileSampleCnt = 0;
	isFirstNode = 1;
	sampleReCount = 0;
	...
	{
		readStatus = fread( &int32TempBuf96, 4, 32, fpRaw );
		if(readStatus != 0){
			nCnt++;
			if( isFirstNode ) {
...
/* we need not this function. Becaus we just used 1 module for one diagnostic .
	onceOpen=1;                 We need this function   */
...
int channelAverageForECE()
{
	...
	int dma_cnt, dmaBlock_size;
/* I will use channel from 96ch to 32 ch.  */
	FILE *fpData[32];
	FILE *fpDataAvr[32];
...
/*	nSkipCnt = 100 / nSampleClk;  Max 100KHz HALPHA Default  */
	fprintf(stdout, "\nMDSplus >>> Sample clock: %dKHz, nTimeGap: %d, nSkipCnt:%d\n", nSampleClk, nTimeGap, nSkipCnt );
...
int dataChannelization()
{
	...
	unsigned int i, j;
	int dma_cnt, dmaBlock_size;
	int int32TempBuf=0, int32TempData96[32];
...
/* I will use channel from 96ch to 32 ch.  */
	...
	{
		fread( &int32TempData96, 4, 32, fpRaw ); 
		for( j=0; j< used_channel; j++) 
		{
/*			fread( &int32TempBuf, 4, 1, fpRaw );  */
#if 0
