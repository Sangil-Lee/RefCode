...
static const iocshArg createChannelArg3 = {"offset val", iocshArgDouble};
static const iocshArg createChannelArg4 = {"gain val", iocshArgDouble};
static const iocshArg* const createChannelArgs[] = { &createChannelArg0,
							 &createChannelArg1,
							 &createChannelArg2,
							 &createChannelArg3,
							 &createChannelArg4};
...
*/
	...
	while(TRUE) {
		...
		if(statusVal ==1){
			...
			/* We will not use this function. Because we have new faction by SNL code. */
#if 0
			...
			printf("Errof Return doCacheInvalidate value is not 1. TGL ############3333 \n");
			drvM6802_set_DAQstop(ptaskConfig);
                }
...
/*	if(args[3].sval) channelOffset = strtof(args[3].sval,NULL);
	if(args[4].sval) channelGain = strtof(args[4].sval,NULL);
*/
	...
	if(!strcmp(task_name,"master")){
		channelOffsetVal[channel_ID]= args[3].dval;
		channelGainVal[channel_ID] = args[4].dval;
		epicsPrintf("Value Check Offset : %f, Gain : %f. \n",args[3].dval,args[4].dval);
	}else if(!strcmp(task_name,"Dev2")){
		channelOffsetVal[channel_ID+32]= args[3].dval;
		channelGainVal[channel_ID+32] = args[4].dval;
		epicsPrintf("Value Check Offset : %f, Gain : %f. \n",args[3].dval,args[4].dval);
	}else if(!strcmp(task_name,"Dev3")){
		channelOffsetVal[channel_ID+64]= args[3].dval;
		channelGainVal[channel_ID+64] = args[4].dval;
		epicsPrintf("Value Check Offset : %f, Gain : %f. \n",args[3].dval,args[4].dval);
	}else epicsPrintf("Devie and channel mismatch.. so you must check the Number.... \n");
...
LOCAL long drvM6802_io_report(int level)
{
	...
	if(level>4 ) {
		epicsPrintf("   ####################### Offset And Gain #############\n");
		epicsPrintf(" TGLEE  ");

		for( i=0; i<32 ; i++) 
		{
			epicsPrintf("Ch%d : OffsetVal:%f, Gain:%f.   \n",i,channelOffsetVal[i],channelGainVal[i]); 
		}
		epicsPrintf("\n");
	}
