...
set_pass0_restoreFile("hrs_savelist.sav")

# specify directories in which to to search for included request files
set_requestfile_path("$(TOP)/iocBoot/$(IOC)", "")
set_requestfile_path("$(TOP)/hrsApp/Db", "")

dbLoadRecords("$(TOP)/db/save_restoreStatus.db", "P=as:")
dbLoadRecords("$(TOP)/db/hrs.db", "P=as:")
dbLoadRecords("$(TOP)/db/status.db")
dbLoadTemplate("$(TOP)/db/hrs_io.template")
