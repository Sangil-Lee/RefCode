...
long mdsPlusInit( genSubRecord *pgsub )
{
    ...
    if(isFirst) {
        ...
        timeBase = (float *)calloc(2047, sizeof(float));
...
long mdsPlusCreatePulse()
{
    ...
    if( status_ok(mdsstat) ) {
        ...
        sscanf(buf,"Current shot is %d", &shotID);
        if(shotID != nextShotID ) {
        ...
        sprintf(buf, "TCL(\"CREATE PULSE %d\")", shotID);
        status = MdsValue(buf,&idesc,&null,&len);
        if(genSubDebug > 2)
            printf("genSub: mdsPlusCreatePulse() MdsValue %s\n",buf);
    }
    ...
    while(1) {
        ...
        if(startMdsPut) {
            ...
            for(j = 0;j < numNode; j++) {
                ...
                /* put data */
                timeBase = (float *)realloc(timeBase, tnoRd*sizeof(float));
                for(i=0;i<tnoRd;i++)
