...
enum gE_NETFAIL {
	GAWAY=1, CHANNEL=2, DATA=4, BACKUP=8, WEB=16, MDSPLUS=128, GAWAY2=256,
	OPI01=GAWAY2*2, OPI02=OPI01*2, OPI03=OPI02*2, OPI04=OPI03*2, OPI05=OPI04*2, OPI06=OPI05*2, 
	OPI07=OPI06*2, SWITCH=OPI07*2 
};
...
static long PingSubProc (subRecord *precord)
{
	...
	static unsigned int prevsvrfail = 0;
	//static int count = 0;
#if 1
	...
	for(setRIter = setreplyAddr.begin(); setRIter != setreplyAddr.end(); ++setRIter, i++)
	{
		...
		if( gvecNetAddrList.at(0).compare(strReSvr) == 0) 
			...
			svrfail &= ~OPI02;
		else if( gvecNetAddrList.at(9).compare(strReSvr) == 0) 
			svrfail &= ~OPI03;
		else if( gvecNetAddrList.at(10).compare(strReSvr) == 0) 
			svrfail &= ~OPI04;
		else if( gvecNetAddrList.at(11).compare(strReSvr) == 0) 
			svrfail &= ~OPI05;
		else if( gvecNetAddrList.at(12).compare(strReSvr) == 0) 
			svrfail &= ~OPI06;
		else if( gvecNetAddrList.at(13).compare(strReSvr) == 0) 
			svrfail &= ~OPI07;
		else if( gvecNetAddrList.at(14).compare(strReSvr) == 0) 
			svrfail &= ~SWITCH;
#if 0
		else if( gvecNetAddrList.at(7).compare(strReSvr) == 0) 
			svrfail &= ~TEST;
	...
	for( setIterFail= resultAddr.begin();setIterFail != resultAddr.end();++setIterFail)
	{
		...
		if( gvecNetAddrList.at(0).compare(strFailSvr) == 0) 
			...
			svrfail |= OPI02;
		else if( gvecNetAddrList.at(9).compare(strFailSvr) == 0) 
			svrfail |= OPI03;
		else if( gvecNetAddrList.at(10).compare(strFailSvr) == 0) 
			svrfail |= OPI04;
		else if( gvecNetAddrList.at(11).compare(strFailSvr) == 0) 
			svrfail |= OPI05;
		else if( gvecNetAddrList.at(12).compare(strFailSvr) == 0) 
			svrfail |= OPI06;
		else if( gvecNetAddrList.at(13).compare(strFailSvr) == 0) 
			svrfail |= OPI07;
		else if( gvecNetAddrList.at(14).compare(strFailSvr) == 0) 
			svrfail |= SWITCH;
#if 0
		else if( gvecNetAddrList.at(7).compare(strFailSvr) == 0) 
			svrfail |= TEST;
	...
	if( svrfail != 0 && svrfail == prevsvrfail)
	{
		precord->val = svrfail;
	};
	prevsvrfail = svrfail;
...
#else
	gsetrequestAddr.insert("172.17.100.198"); 
	gsetrequestAddr.insert("172.17.100.199");
	gsetrequestAddr.insert("172.17.100.200");
	gsetrequestAddr.insert("172.17.100.201");
	gsetrequestAddr.insert("172.17.100.202");
	gsetrequestAddr.insert("172.17.100.203");

	//Test OPI
	gsetrequestAddr.insert("172.17.100.101");
	gsetrequestAddr.insert("172.17.100.102");
	gsetrequestAddr.insert("172.17.100.103");
	gsetrequestAddr.insert("172.17.100.104");
	gsetrequestAddr.insert("172.17.100.105");
	gsetrequestAddr.insert("172.17.100.106");
	gsetrequestAddr.insert("172.17.100.107");
	gsetrequestAddr.insert("203.230.120.214");

	gvecNetAddrList.push_back("172.17.100.198"); 
	gvecNetAddrList.push_back("172.17.100.199");
	gvecNetAddrList.push_back("172.17.100.200");
	gvecNetAddrList.push_back("172.17.100.201");
	gvecNetAddrList.push_back("172.17.100.202");
	gvecNetAddrList.push_back("172.17.100.203");
	//Test OPI
	gvecNetAddrList.push_back("172.17.100.101");
	gvecNetAddrList.push_back("172.17.100.102");
	gvecNetAddrList.push_back("172.17.100.103");
	gvecNetAddrList.push_back("172.17.100.104");
	gvecNetAddrList.push_back("172.17.100.105");
	gvecNetAddrList.push_back("172.17.100.106");
	gvecNetAddrList.push_back("172.17.100.107");
	gvecNetAddrList.push_back("203.230.120.214");
#endif
	return 0;
}
