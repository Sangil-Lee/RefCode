...
/*subroutine record*/
static long subCtrlRoomTempInit(subRecord *precord)
{
	    return 0;
}

static long subCtrlRoomTempCalc(subRecord *precord)
{
	...
	if( pcalcRecordLinkA->val >= pcalcRecordLinkA->hihi ||
		...
		pcalcRecordLinkE->val >= pcalcRecordLinkE->hihi ||
		pcalcRecordLinkF->val >= pcalcRecordLinkF->hihi ||
		pcalcRecordLinkG->val >= pcalcRecordLinkG->hihi )
	{
		precord ->val = 1;
	}
	else
...
epicsRegisterFunction(subCtrlRoomTempInit);
epicsRegisterFunction(subCtrlRoomTempCalc);
