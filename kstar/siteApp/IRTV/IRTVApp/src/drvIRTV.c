...
void threadFunc_IRTV_DAQ(void *param)
{
	...
	while (TRUE) {
		...
		kLog (K_INFO, "[IRTV_DAQ] mode(%s) rate(%.f) t0(%.f) t1(%.f) grap(%.f) time(%f) shot(%d) sync(%.f) status(%s)\n",
				getModeMsg(pSTDdev->ST_Base.opMode),
				pIRTV->sample_rate, 
				pIRTV->start_t0, pIRTV->stop_t1, pIRTV->ao_grab_time, pIRTV->sample_time,
				pMaster->ST_Base.shotNumber, pIRTV->ao_sync_source,
		...
		if (pSTDdev->StatusDev & TASK_STANDBY) {
			...
			// 일정 시간 동안, Camera의 상태를 읽어 내부 변수 초기화
			int i;
			for (i = 0; i < 5; i++) {
				if (OK == irtvGetCameraStatus (pIRTV)) {
					copyStatusToPVs (pIRTV);
