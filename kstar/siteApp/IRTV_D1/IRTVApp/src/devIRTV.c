...
static void devIRTV_BO_TRIGGER_REARM(ST_execParam *pParam)
{
	kLog (K_MON, "[devIRTV_BO_TRIGGER_REARM] bo_trigger_rearm(%f)\n", pParam->setValue);

	if (0 == pParam->setValue) {
		return;
	}

	kLog (K_MON, "[devIRTV_BO_TRIGGER_REARM] before ... \n");

	irtvSendTriggerRearm ();

	kLog (K_MON, "[devIRTV_BO_TRIGGER_REARM] after  ... \n");

	DBproc_put (sfwEnvGet("PV_NAME_TRIGGER_REARM"), "0");
}
...
static void devIRTV_AO_TRIGGER_MODE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev	= get_first_STDdev();

	kLog (K_MON, "[devIRTV_AO_TRIGGER_MODE] ao_trigger_mode(%f)\n", pParam->setValue);

	if (TRUE != assertNonArmEnabled ("devIRTV_AO_TRIGGER_MODE", pSTDdev)) {
		return;
	}

	if (getDriver(pSTDdev)->ao_trigger_mode == pParam->setValue) {
		return;
	}

	getDriver(pSTDdev)->ao_trigger_mode	= pParam->setValue;

	kLog (K_MON, "[devIRTV_AO_TRIGGER_MODE] before ... \n");

	irtvSetTriggerMode ((int)pParam->setValue);

	kLog (K_MON, "[devIRTV_AO_TRIGGER_MODE] after  ... \n");
}
...
static void devIRTV_AO_GAIN_LEVEL(ST_execParam *pParam)
{
	...
	irtvSetGainLevel ((int)getDriver(pSTDdev)->ao_gain_level);
...
static void devIRTV_BO_SYNC_POLARITY(ST_execParam *pParam)
{
	...
	kLog (K_MON, "[devIRTV_BO_SYNC_POLARITY] bo_sync_polarity(%f -> %f)\n", getDriver(pSTDdev)->bo_sync_polarity, pParam->setValue);
...
static void devIRTV_BO_PRESET_SEQ_MODE(ST_execParam *pParam)
{
	...
	getDriver(pSTDdev)->bo_preset_seq_mode	= (int)pParam->setValue;
...
static void devIRTV_BO_GEN_SEQ_FILE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev	= get_first_STDdev();
	ST_IRTV			*pIRTV		= getDriver(pSTDdev);

	kLog (K_MON, "[devIRTV_BO_GEN_SEQ_FILE] ...\n");

	pIRTV->bo_gen_seq_file	= (int)pParam->setValue;

	kLog (K_MON, "[devIRTV_BO_GEN_SEQ_FILE] after  ... \n");
}

static void devIRTV_BO_GEN_TIFF_FILE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev	= get_first_STDdev();
	ST_IRTV			*pIRTV		= getDriver(pSTDdev);

	kLog (K_MON, "[devIRTV_BO_GEN_TIFF_FILE] ...\n");

	pIRTV->bo_gen_tiff_file	= (int)pParam->setValue;

	kLog (K_MON, "[devIRTV_BO_GEN_TIFF_FILE] after  ... \n");
}

static void devIRTV_BO_GEN_AVI_FILE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev	= get_first_STDdev();
	ST_IRTV			*pIRTV		= getDriver(pSTDdev);

	kLog (K_MON, "[devIRTV_BO_GEN_AVI_FILE] ...\n");

	pIRTV->bo_gen_avi_file	= (int)pParam->setValue;

	kLog (K_MON, "[devIRTV_BO_GEN_AVI_FILE] after  ... \n");
}

static void devIRTV_BO_CONN_ENABLE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev	= get_first_STDdev();
	ST_IRTV			*pIRTV		= getDriver(pSTDdev);

	kLog (K_MON, "[devIRTV_BO_CONN_ENABLE] ...\n");

	pIRTV->bo_conn_enable	= (int)pParam->setValue;

	kLog (K_MON, "[devIRTV_BO_CONN_ENABLE] after  ... \n");
}
...
static long devAoIRTV_init_record(aoRecord *precord)
{
	...
	else if (!strcmp(pSTdpvt->arg0, AO_TRIGGER_MODE_STR)) 	{ pSTdpvt->ind = AO_TRIGGER_MODE; }
	...
	if(precord->pact == FALSE) {	
		...
		switch (pSTdpvt->ind) {
			...
			case AO_SYNC_SOURCE:
				...
				break;
			case AO_TRIGGER_MODE:
				qData.pFunc = devIRTV_AO_TRIGGER_MODE;
				break;
...
static long devAiIRTV_init_record(aiRecord *precord)
{
	...
	else if (!strcmp (pSTdpvt->arg0, AI_FRONT_TEMP_STR)) {
		pSTdpvt->ind		= AI_FRONT_TEMP;
	}
	else if (!strcmp (pSTdpvt->arg0, AI_AIRGAP_TEMP_STR)) {
		pSTdpvt->ind		= AI_AIRGAP_TEMP;
	}
	else if (!strcmp (pSTdpvt->arg0, AI_INTERNAL_TEMP_STR)) {
		pSTdpvt->ind		= AI_INTERNAL_TEMP;
	}
...
{ 
	...
	switch (pSTdpvt->ind) {
		...
		case AI_FRONT_TEMP :
			precord->val = pIRTV->ai_sts_front_temp;
			kLog (K_DATA, "[devAiIRTV_read_ai.AI_FRONT_TEMP] %s : %f\n", pSTDdev->taskName, precord->val);
			break;

		case AI_AIRGAP_TEMP :
			precord->val = pIRTV->ai_sts_airgap_temp;
			kLog (K_DATA, "[devAiIRTV_read_ai.AI_AIRGAP_TEMP] %s : %f\n", pSTDdev->taskName, precord->val);
			break;

		case AI_INTERNAL_TEMP :
			precord->val = pIRTV->ai_sts_internal_temp;
			kLog (K_DATA, "[devAiIRTV_read_ai.AI_INTERNAL_TEMP] %s : %f\n", pSTDdev->taskName, precord->val);
			break;
...
static long devBoIRTV_init_record(boRecord *precord)
{
	...
	else if (!strcmp(pSTdpvt->arg0, BO_TRIGGER_REARM_STR)) {
		pSTdpvt->ind = BO_TRIGGER_REARM;
	} 
	...
	else if (!strcmp(pSTdpvt->arg0, BO_GEN_SEQ_FILE_STR)) {
		pSTdpvt->ind = BO_GEN_SEQ_FILE;
	} 
	else if (!strcmp(pSTdpvt->arg0, BO_GEN_TIFF_FILE_STR)) {
		pSTdpvt->ind = BO_GEN_TIFF_FILE;
	} 
	else if (!strcmp(pSTdpvt->arg0, BO_GEN_AVI_FILE_STR)) {
		pSTdpvt->ind = BO_GEN_AVI_FILE;
	} 
	else if (!strcmp(pSTdpvt->arg0, BO_CONN_ENABLE_STR)) {
		pSTdpvt->ind = BO_CONN_ENABLE;
	} 
	else {
	...
	if(precord->pact == FALSE) {	
		...
		switch(pSTdpvt->ind) {
			case BO_SOFT_TRIGGER:
				...
				break;
			case BO_TRIGGER_REARM:
				qData.pFunc = devIRTV_BO_TRIGGER_REARM;
				break;
			case BO_LIVE_VIEW:
			...
			case BO_GET_STORED_NUCS:
				...
				break;
			case BO_GEN_SEQ_FILE:
				qData.pFunc = devIRTV_BO_GEN_SEQ_FILE;
				break;
			case BO_GEN_AVI_FILE:
				qData.pFunc = devIRTV_BO_GEN_AVI_FILE;
				break;
			case BO_CONN_ENABLE:
				qData.pFunc = devIRTV_BO_CONN_ENABLE;
				break;
			default: 
				qData.pFunc = NULL;
				break;
...
{ 
	...
	switch (pSTdpvt->ind) {
		case SI_SYNC_SRC_NAME :
			...
			else if (2 == pIRTV->mbbi_sts_sync_src)	strcpy (precord->val, "SW");
