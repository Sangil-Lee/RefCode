...
/* auto Run function */
static void devNI6254_BO_AUTO_RUN(ST_execParam *pParam)
{
}

static void devNI6254_BO_DAQ_STOP(ST_execParam *pParam)
{
	kuDebug (kuMON, "[devNI6254_BO_DAQ_STOP] daq_stop(%f)\n", pParam->setValue);
...
static void devNI6254_BO_CH_ENABLE(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev = pParam->pSTDdev;
	ST_NI6254		*pNI6254 = pSTDdev->pUser;
	int	chNo	= pParam->n32Arg0;

	kuDebug (kuMON, "[devNI6254_BO_CH_ENABLE] %s : ch(%d) enable(%d -> %f)\n",
			pSTDdev->taskName, chNo, pNI6254->ST_Ch[chNo].enabled, pParam->setValue);

	pNI6254->ST_Ch[chNo].enabled = pParam->setValue;

	scanIoRequest(pSTDdev->ioScanPvt_userCall);
}

static void devNI6254_AO_DAQ_MIN_VOLT(ST_execParam *pParam)
{
	ST_MASTER *pMaster = get_master();
	ST_STD_device *pSTDdev = (ST_STD_device*) ellFirst(pMaster->pList_DeviceTask);
	ST_NI6254 *pNI6254 = NULL;

	kuDebug (kuMON,"[devNI6254_AO_DAQ_MIN_VOLT] minVolt(%f)\n", pParam->setValue);

	while(pSTDdev) {
		if (pSTDdev->StatusDev & TASK_ARM_ENABLED) {
			kuDebug (kuDATA,"[devNI6254_AO_DAQ_MIN_VOLT] %s: System is armed! \n", pSTDdev->taskName);
			notify_error (1, "System is armed!", pSTDdev->taskName );
		}
		else {
			pNI6254 = pSTDdev->pUser;
			pNI6254->minVal = pParam->setValue;
			scanIoRequest(pSTDdev->ioScanPvt_userCall);

			kuDebug (kuDATA,"[devNI6254_AO_DAQ_MIN_VOLT] task(%s) minVolt(%.f)\n",
					pSTDdev->taskName, pNI6254->minVal);
		}

		pSTDdev = (ST_STD_device*) ellNext(&pSTDdev->node);
	}
}
static void devNI6254_AO_DAQ_MAX_VOLT(ST_execParam *pParam)
{
	ST_MASTER *pMaster = get_master();
	ST_STD_device *pSTDdev = (ST_STD_device*) ellFirst(pMaster->pList_DeviceTask);
	ST_NI6254 *pNI6254 = NULL;

	kuDebug (kuMON,"[devNI6254_AO_DAQ_MAX_VOLT] maxVolt(%f)\n", pParam->setValue);

	while(pSTDdev) {
		if (pSTDdev->StatusDev & TASK_ARM_ENABLED) {
			kuDebug (kuDATA,"[devNI6254_AO_DAQ_MAX_VOLT] %s: System is armed! \n", pSTDdev->taskName);
			notify_error (1, "System is armed!", pSTDdev->taskName );
		}
		else {
			pNI6254 = pSTDdev->pUser;
			pNI6254->maxVal = pParam->setValue;
			scanIoRequest(pSTDdev->ioScanPvt_userCall);

			kuDebug (kuDATA,"[devNI6254_AO_DAQ_MAX_VOLT] task(%s) maxVolt(%.f)\n",
					pSTDdev->taskName, pNI6254->maxVal);
		}
...
static void devNI6254_AO_DAQ_GAIN(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev = pParam->pSTDdev;
	ST_NI6254		*pNI6254 = pSTDdev->pUser;
	int				chNo	 = pParam->n32Arg0;

	kuDebug (kuMON, "[devNI6254_AO_DAQ_GAIN] %s : ch(%d) gain(%f -> %f)\n",
			pSTDdev->taskName, chNo, pNI6254->ST_Ch[chNo].gain, pParam->setValue);

	pNI6254->ST_Ch[chNo].gain = pParam->setValue;

	scanIoRequest(pSTDdev->ioScanPvt_userCall);
}

static void devNI6254_AO_DAQ_OFFSET(ST_execParam *pParam)
{
	ST_STD_device	*pSTDdev = pParam->pSTDdev;
	ST_NI6254		*pNI6254 = pSTDdev->pUser;
	int				chNo	 = pParam->n32Arg0;

	kuDebug (kuMON, "[devNI6254_AO_DAQ_OFFSET] %s : ch(%d) offset(%f -> %f)\n",
			pSTDdev->taskName, chNo, pNI6254->ST_Ch[chNo].offset, pParam->setValue);

	pNI6254->ST_Ch[chNo].offset = pParam->setValue;
...
static long devAoNI6254_init_record(aoRecord *precord)
{
	...
	switch(precord->out.type) {
		case INST_IO:
			...
			kuDebug (kuINFO, "[devAoNI6254_init_record] %d: %s %s %s\n",
...
static long devAoNI6254_write_ao(aoRecord *precord)
{
	...
	if(precord->pact == FALSE) {	
		...
		kuDebug (kuDEBUG, "[devAoNI6254_write_ao] db processing: phase I %s (%s)\n", precord->name,
				epicsThreadGetNameSelf());

		switch(pSTdpvt->ind) {
			case AO_DAQ_MIN_VOLT:
				qData.pFunc = devNI6254_AO_DAQ_MIN_VOLT;
				break;
			case AO_DAQ_MAX_VOLT:
				qData.pFunc = devNI6254_AO_DAQ_MAX_VOLT;
				break;
			case AO_DAQ_GAIN:
				qData.pFunc = devNI6254_AO_DAQ_GAIN;
				break;
			case AO_DAQ_OFFSET:
				qData.pFunc = devNI6254_AO_DAQ_OFFSET;
				break;
		}
	...
	if(precord->pact == TRUE) {
		kuDebug (kuDEBUG, "[devAoNI6254_write_ao] db processing: phase II %s (%s)\n",
...
static long devAiNI6254_init_record(aiRecord *precord)
{
	...
	switch(precord->inp.type) {
		case INST_IO:
			...
			kuDebug (kuINFO, "[devAiNI6254_init_record] %d: %s %s %s\n",
					i, pSTdpvt->devName, pSTdpvt->arg0, pSTdpvt->arg1);
	...
	if (2 == i) {
		...
		kuDebug (kuERR, "[devAiNI6254_init_record] arg cnt (%d) \n",  i );
...
{ 
	...
	switch (pSTdpvt->ind) {
		case AI_READ_STATE :
			...
			kuDebug (kuDATA, "[devAiNI6254_read_ai.AI_READ_STATE] %s : value(%f)\n", pSTDdev->taskName, precord->val);
			break;

		case AI_RAW_VALUE :
			precord->val = pNI6254->ST_Ch[chId].rawValue;
			kuDebug (kuDATA, "[devAiNI6254_read_ai.AI_RAW_VALUE] %s : ch(%d) raw(%d) (0x%x)\n",
					pSTDdev->taskName, chId, (int)precord->val, (int)precord->val);
			break;

		case AI_VOLT_VALUE :
			precord->val = (float)pNI6254->ST_Ch[chId].euVolt;
			kuDebug (kuDATA, "[devAiNI6254_read_ai.AI_VOLT_VALUE] %s : ch(%d) value(%f) euVolt(%f)\n",
					pSTDdev->taskName, chId, precord->val, pNI6254->ST_Ch[chId].euVolt);
			break;

		case AI_VALUE :
			precord->val = pNI6254->ST_Ch[chId].euValue;
			kuDebug (kuDATA, "[devAiNI6254_read_ai.AI_VALUE] %s : ch(%d) value(%f) euVolt(%f) gain(%f) offset(%f)\n",
					pSTDdev->taskName, chId, precord->val,
					pNI6254->ST_Ch[chId].euVolt, pNI6254->ST_Ch[chId].gain, pNI6254->ST_Ch[chId].offset);
			break;

		case AI_CALC_VALUE :
			precord->val = pNI6254->ST_Ch[chId].calcValue;
			kuDebug (kuDATA, "[devAiNI6254_read_ai.AI_CALC_VALUE] %s : ch(%d) calc(%f) (0x%x)\n",
					pSTDdev->taskName, chId, (int)precord->val, (int)precord->val);
			break;

		default:
			kuDebug (kuERR, "[devAiNI6254_read_ai] %s : ind(%d) is invalid\n", pSTDdev->taskName, pSTdpvt->ind); 
			break;
	}
...
static long devBoNI6254_init_record(boRecord *precord)
{
	...
	switch(precord->out.type) {
		case INST_IO:
			...
			kuDebug (kuMON, "[devBoNI6254_init_record] arg num: %d: %s %s %s\n", i,
					pSTdpvt->devName, pSTdpvt->arg0, pSTdpvt->arg1);
	...
	if (!strcmp(pSTdpvt->arg0, BO_AUTO_RUN_STR)) {
		...
		pSTdpvt->ind = -1;
		kuDebug (kuERR, "ERROR! devBoNI6254_init_record: arg0 \"%s\" \n",  pSTdpvt->arg0 );
	}
...
static long devBoNI6254_write_bo(boRecord *precord)
{
	...
	if(precord->pact == FALSE) {	
		...
		switch(pSTdpvt->ind) {
			case BO_AUTO_RUN:
				qData.pFunc = devNI6254_BO_AUTO_RUN;
				break;
			...
			case BO_CH_ENABLE:
				qData.pFunc = devNI6254_BO_CH_ENABLE;
				break;
...
static long devMbbiNI6254_init_record(mbbiRecord *precord)
{
	ST_dpvt *pSTdpvt = (ST_dpvt*) malloc(sizeof(ST_dpvt));
	int i;
	...
	switch(precord->inp.type) {
		case INST_IO:
			...
			kuDebug (kuINFO, "[devMbbiNI6254_init_record] %d: %s\n", i, pSTdpvt->arg0);

			pSTdpvt->pMaster = get_master();
			if (!pSTdpvt->pMaster) {
				recGblRecordError(S_db_badField, (void*) precord,
						"devMbbiNI6254 (init_record) Illegal INP field: task name");
				free(pSTdpvt);
				precord->udf = TRUE;
				precord->dpvt = NULL;
				return S_db_badField;
		...
		default:
			recGblRecordError(S_db_badField, (void*) precord,
					"devMbbiNI6254 (init_record) Illegal INP field");
			free(pSTdpvt);
			precord->udf = TRUE;
			precord->dpvt = NULL;
	...
	if (!strcmp (pSTdpvt->arg0, MBBI_DIG_STAT_STR)) {
		...
		kuDebug (kuERR, "[devMbbiNI6254_init_record] arg cnt (%d) \n",  i );
		return -1;	/*returns: (-1,0)=>(failure,success)*/
	}
...
static long devMbbiNI6254_get_ioint_info(int cmd, mbbiRecord *precord, IOSCANPVT *ioScanPvt)
{
	ST_dpvt *pSTdpvt = (ST_dpvt*) precord->dpvt;
	ST_MASTER	*pMaster;
...
static long devMbbiNI6254_read_mbbi(mbbiRecord *precord)
{ 
	ST_dpvt	*pSTdpvt = (ST_dpvt*) precord->dpvt;
	ST_MASTER	*pMaster;
	...
	switch (pSTdpvt->ind) {
		default:
			kuDebug (kuERR, "[devMbbiNI6254_read_mbbi] ind(%d) is invalid\n", pSTdpvt->ind); 
			break;
	}
...
BINARYDSET	devMbbiNI6254Read  = { 6, NULL, NULL, devMbbiNI6254_init_record, devMbbiNI6254_get_ioint_info, devMbbiNI6254_read_mbbi, NULL };
...
epicsExportAddress(dset, devMbbiNI6254Read);
