...
static int send_dataChannel(drvVXI1564a_taskConfig *ptaskConfig);
...
static void set_shotNumber(drvVXI1564a_taskConfig *ptaskConfig, uInt32 shotNumber);
static void set_blipTime(drvVXI1564a_taskConfig *ptaskConfig, float64 blipTime);
static void set_daqTime(drvVXI1564a_taskConfig *ptaskConfig, float64 blipTime);
...
static void set_taskExTrigger(drvVXI1564a_taskConfig *ptaskConfig, unsigned int exTrig)
{
	...
	ptaskConfig->opMode = exTrig ;       /* Info about the Remote Data Store at External Trigger and Soft Trig Data Store at Local Storage  */
	if(exTrig==0)
	{
		...
		ptaskConfig->triggerSource = tempTrig | 0x80;   /* 7 bit Enable for Soft Trig */
		ptaskConfig->sampleSource = tempSample | 0x21A0;  /* 5, 7, 8, 13 bit Enable for Soft Trigger   */
	}else if(exTrig==1){
		...
		ptaskConfig->sampleSource = tempSample | 0x20A0;  /* This Register setting for sample control. so, same soft and ex-trigger state */
...
#endif
		semStatus=sem_post( &DAQsemaphore );
		epicsPrintf("Semaphore Post Done : %d \n ",semStatus); 
	}
	else {
...
static int send_dataChannel(drvVXI1564a_taskConfig *ptaskConfig)
{
	...
	uInt32 i;
	float64  sampleCounts;
	
	unsigned int chNu;	
   	/* Event type is a number which identifies the event type that has been received. */
	
	sampleCounts = ptaskConfig->sampleCount + ptaskConfig->samplePreCount;

	data1 = (float *)malloc(sizeof(float)*sampleCounts);
	data2 = (float *)malloc(sizeof(float)*sampleCounts);
	data3 = (float *)malloc(sizeof(float)*sampleCounts);
	data4 = (float *)malloc(sizeof(float)*sampleCounts);
...
#if 1
	for(i=sampleCounts -10; i<sampleCounts; i++){
			printf("  %f, ",(double)data1[i]*4/32768);
	}
	printf("\n");
	printf("\n");
#endif
#if 0
	epicsPrintf("taskName : %s, Sample Count %f, Pre Count : %f \n",ptaskConfig->taskName, ptaskConfig->sampleCount,ptaskConfig->samplePreCount);
	epicsPrintf("taskName2 : %s, sizeof data1 : %d \n",ptaskConfig->taskName,sizeof(data1));
	epicsThreadSleep(2.0);
	epicsPrintf("taskName3 : %s, Sample Count3 %f, Pre Count3 : %f \n",ptaskConfig->taskName,ptaskConfig->sampleCount,ptaskConfig->samplePreCount);
	epicsThreadSleep(2.0);
#endif
...
#if 0
		epicsPrintf("channel Number :%d , sample counts : %f .paoChannel\n ",chNu, sampleCounts);
		epicsPrintf("channel Number :%d , data0 : %d \n ",chNu,(signed short)data1[0]);
		epicsPrintf("channel Number :%d , sizeof data : %d, i: %d \n ",chNu,sizeof(malloc(sizeof(signed short)*sampleCounts)),i);
		epicsPrintf("channel Number :%d , bufferSize : %d \n ",chNu,sizeof(paoChannelConfig->vxi1564aChBuffer));
		epicsThreadSleep(2.0);
		case 0 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data1, sizeof(signed short)*sampleCounts);     break; 
    		case 1 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data2, sizeof(signed short)*sampleCounts);     break; 
    		case 2 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data3, sizeof(signed short)*sampleCounts);     break; 
    		case 3 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data4, sizeof(signed short)*sampleCounts);     break; 
#endif
	switch(chNu) {
		case 0 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data1, sizeof(float)*sampleCounts);     break; 
    		case 1 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data2, sizeof(float)*sampleCounts);     break; 
    		case 2 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data3, sizeof(float)*sampleCounts);     break; 
    		case 3 :  memcpy(paoChannelConfig->vxi1564aChBuffer, data4, sizeof(float)*sampleCounts);     break; 
	...
	epicsPrintf("Channelization from device memory : Current Time %lf msec \n", 1.E-3 * (double)drvVXI1564a_intervalUSec(start, stop));
	return 1;
}
...
#ifdef DEBUG
		epicsThreadSleep(0.1);
		epicsPrintf("MDSplus Data Closing Shot Number : %ld --- Remote Mode!!! \n", shotNumber);
...
#ifdef DEBUG
		epicsThreadSleep(0.1);
		epicsPrintf("MDSplus Data Closing Shot Number : %ld --- Local Mode!!! \n",shotNumber);
#endif
	}
	return 1;
}
...
int completeDataPut()
{
        ...
        if( !statusOk(status) )
        {
                epicsPrintf(stderr,"Error TCL command : create pulse: %s.\n", MdsGetMsg(status));
                return 0;
        }
...
*/
                ...
                epicsPrintf("\%s >>> Create new tree(\"%ld\")... OK\n", ptaskConfig->taskName, shotNumber);
       	}
...
*/
	...
	ptaskConfig->samplingRate = 1./samplePeriod;
		
        tempHighRate= 10000000 * samplePeriod; /* Internal Clock Source 10M - 10000000  */
        tempLowRate=  10000000 * samplePeriod;

        ptaskConfig->sampleRateHigh = (tempHighRate & 0xFF0000) >> 16;
        ptaskConfig->sampleRateLow = tempLowRate & 0xFFFF;

#ifdef DEBUG
	epicsPrintf("set_taskTiming:Name: %s, Rate:%f (Hz),Period(Interval):%f (sec), Counts:%f, chanNum:%d\n",
...
/*  Epics Sleep Function is defending Create Before complate the pcontrolStoreConfig        */
	...
	semStatus=sem_init( &DAQsemaphore, 0, 0);
	while(TRUE) {
		semStatus = sem_wait( &DAQsemaphore );
...
#if 0        /* Remove this function because it is not reseasonable for our each module alone test  TG.Lee */
		ptaskConfig->dataPutDone = 1;  /* Data store action complated in MDSplus Db  */
		if(!strcmp(ptaskConfig->taskName, "VXI0::40::INSTR")) 
    		{
			dataPutComplate =1;
			while(dataPutComplate==1){
				ptaskConfig = (drvVXI1564a_taskConfig*) ellFirst(pdrvVXI1564aConfig->ptaskList);
				dataPutDoneCounts =0;
				while(ptaskConfig) {
  					dataPutDoneCounts += ptaskConfig->dataPutDone;
					ptaskConfig = (drvVXI1564a_taskConfig*) ellNext(&ptaskConfig->node);
				}
				if(ellCount(pdrvVXI1564aConfig->ptaskList) !=dataPutDoneCounts){
					epicsPrintf("VXI Counts :%d, data store does not complated. so I Work!! \n",dataPutDoneCounts);
				} else {
					ptaskConfig = (drvVXI1564a_taskConfig*) ellFirst(pdrvVXI1564aConfig->ptaskList);
					dataPutComplate=0;
...
/* Protection Continue mesage Queue.When data store complated We Mem refrish and then eventDone Value is Zero return.  */
		epicsMessageQueueSend(controlStoreQueueID, (void*) &queueData, sizeof(controlThreadQueueData));
		pFunc      = queueData.pFunc;
	        pexecParam = &queueData.param;
		ptaskConfig = (drvVXI1564a_taskConfig *) pexecParam->ptaskConfig;
		epicsPrintf(" MDSplus Data Put Start. Task Name %s . \n", ptaskConfig->taskName);
...
*/
			epicsPrintf("   SamplePeriod Min : %f/sec, Sample Max Counts: %f, Pre-Sample Conuts:%f \n",
							 ptaskConfig->samplePeriodMin, ptaskConfig->sampleMaxCount, ptaskConfig->samplePreCount );
			epicsPrintf("   Sampling Rate: %f/sec, Sample Counts: %f, Pre-Sample Conuts:%f \n",
							 ptaskConfig->samplingRate, ptaskConfig->sampleCount, ptaskConfig->samplePreCount );
			epicsPrintf("  ShotNumber : %lu, Blip Time: %f, DAQ start Time :%f \n",
...
#endif
void drvVXI1564a_set_shotNumber(drvVXI1564a_taskConfig *ptaskConfig, uInt32 shotNumber)
{
	set_shotNumber(ptaskConfig, (uInt32) shotNumber);
	return;
}
...
void drvVXI1564a_set_daqTime(drvVXI1564a_taskConfig *ptaskConfig, double blipTime)
{
	set_daqTime(ptaskConfig, (float64) blipTime);
	return;
}
