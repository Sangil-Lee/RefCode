...
{ 
	return (pNI6123->sample_rate / 10); // Every event per 10msec for real time monitoring the value.
}
...
{
/* Counts to Number of channel per each Device */
...
int getTotSampsAllChan (ST_STD_device *pSTDdev) 
{
	...
	return (pNI6123->sample_rate * getMaxNumChan(pSTDdev)); 
}
...
int createLocalMdsTree (ST_STD_device *pSTDdev)
{
	...
	return (WR_ERROR);
#else
	// local tree is created by MMWI_DAQ

	kLog (K_MON, "[createLocalMdsTree] %s : local tree is created by MMWI\n", pSTDdev->taskName);
...
/*
	if (TRUE == isPrintDebugMsg (K_DATA)) {
		for (i=0;i<numSampsPerChan;i++) {
			if ( (i > 0) && ! (i % 10)) printf ("\n");
			printf ("%.2f ", pNI6123->fScaleReadArray[i*chanNum]);
		}
		printf ("\n");
	}
*/
	...
	if (TRUE == isPrintDebugMsg (K_DATA)) {
		for (i=0;i<numSampsPerChan;i++) {
			if ( (i > 0) && ! (i % 10)) printf ("\n");
			printf ("%d ", pNI6123->readArray[i*chanNum]);
		}
		printf ("\n");
	}
...
*/
		pNI6123->ST_Ch[chNo].rawValue  = pNI6123->readArray[arrayJumpCurrent];
		pNI6123->ST_Ch[chNo].euValue  = pNI6123->fScaleReadArray[arrayJumpCurrent];
...
*/
	...
	epicsPrintf("[ EveryNCallback ] : measured function and  processed and write data file time sec. is %lf msec\n", 1.E-3 * (double)wf_intervalUSec(start,stop));
...
void processConstantDensity(ST_NI6123 *pNI6123, int chNo)
{
	int i;
	float64 dataTemp;
	float64 conA = pNI6123->ST_Ch[chNo].conA;
	float64 conB = pNI6123->ST_Ch[chNo].conB;
	float64 conC = pNI6123->ST_Ch[chNo].conC;
	float64 conD = pNI6123->ST_Ch[chNo].conD;

	int32	numSampsPerChan = getNumSampsPerChan(pNI6123);
	/* Calc the density from raw scale data to density param value for each channel before density calc between 2 channels  */
	for (i=0; i<numSampsPerChan; i++)
	{	dataTemp = pNI6123->ST_Ch[chNo].fScaleReadChannel[i];
		pNI6123->ST_Ch[chNo].fScaleReadChannel[i] = conA*((conB*(dataTemp*dataTemp)) + (conC * dataTemp) + conD); 
	}
...
int processDensity (ST_NI6123 *pNI6123, int chNo)
{
	...
	int32	numSampsPerChan = getNumSampsPerChan(pNI6123);
/* herer TG */ 
	processConstantDensity(pNI6123, chNo);
	processConstantDensity(pNI6123, chNo+1);
	...
	if(pNI6123->ST_Ch[chNo].isFirst == 0) {
		...
		kLog (K_INFO,"[threadFunc_processDensity_isFirst ] Density first value : %f , avgNES1 : %f,  avgNES2 : %f \n", 
				pNI6123->ST_Ch[chNo].density, pNI6123->ST_Ch[chNo].avgNES1, pNI6123->ST_Ch[chNo].avgNES2);
	} else {
	...
	for (i=0; i<numSampsPerChan; i++)
	{
		...
		/* old - function TG : pNI6123->ST_Ch[chNo].calcDensity[i] = pNI6123->ST_Ch[chNo].density;	  */
		pNI6123->ST_Ch[chNo].calcDensity[i] = pNI6123->ST_Ch[chNo].density + pNI6123->ST_Ch[selectCh].fScaleReadChannel[i];	
		if(i==0){	
			kLog (K_INFO,"[threadFunc_processDensity_is first array[0]] Density first value : %f , CalcDensity : %f,  fringValue : %f \n", 
					pNI6123->ST_Ch[chNo].density, pNI6123->ST_Ch[chNo].calcDensity[i], pNI6123->ST_Ch[selectCh].fScaleReadChannel[i]);
		}
	}
			...
			/*	processAutoTrendRun (pSTDdev);   */
				...
				NIERROR(
					DAQmxCfgSampClkTiming(
						pNI6123->taskHandle,	// taskHandle
						pNI6123->clockSource,	// source terminal of the Sample clock. NULL(internal)
...
int create_NI6123_taskConfig(ST_STD_device *pSTDdev)
{
	...
	kLog (K_TRACE, "[create_NI6123_taskConfig] devType(%s) task(%s) dev(%s) trig(%s) sigType(%s) clock(%s)\n",
		pSTDdev->devType, pSTDdev->taskName, pSTDdev->strArg0, pSTDdev->strArg1, pSTDdev->strArg2, pSTDdev->strArg3);
	...
	/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	chanNum = ellCount(pSTDdev->pList_Channel);
	kLog (K_TRACE, "[create_NI6259_taskConfig] task(%s) dev(%s) trig(%s) sigType(%s) clock(%s) channelCounts(%d)\n",
	...
	sprintf (pNI6123->clockSource, "%s", pSTDdev->strArg3);

	pNI6123->terminal_config	= DAQmx_Val_Diff;	// DAQmx_Val_Diff(6123), DAQmx_Val_NRSE NI-6254(DAQmx_Val_RSE)
	...
	pSTDdev->ST_Func._CREATE_LOCAL_TREE	= func_NI6123_CREATE_LOCAL_SHOT;
		...
		if(level>3) {
			...
			for(i=0;i<chanNum;i++) {
				epicsPrintf(" Ch No:%d, Tag Name : %s, selectCh : %f, density : %f, fringe : %f, dLimUpper :%f, dLimLow : %f \n",
						i, pNI6123->ST_Ch[i].strTagName,pNI6123->ST_Ch[i].selectCh, pNI6123->ST_Ch[i].density, pNI6123->ST_Ch[i].fringe, pNI6123->ST_Ch[i].dLimUpper, pNI6123->ST_Ch[i].dLimLow);
				epicsPrintf("\n");
				epicsPrintf(" Constant A : %f, Constant B : %f, Constant C : %f, Constant D : %f, avgNES1 :%f, avgNES2 : %f \n",
						 pNI6123->ST_Ch[i].conA, pNI6123->ST_Ch[i].conB, pNI6123->ST_Ch[i].conC, pNI6123->ST_Ch[i].conD, pNI6123->ST_Ch[i].avgNES1, pNI6123->ST_Ch[i].avgNES2);
				epicsPrintf("\n");
			}
#endif
			epicsPrintf("   ");
