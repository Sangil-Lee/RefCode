...
int set_up_tcp(char *ip)
{
    ...
    if(sock >= 0) {
        connect_stat = connect(sock, (struct sockaddr *)&server,
...
_RECONNECT:
	tcmds->sock = set_up_tcp(tcmds->ip);
	...
	while(RUN) {
		...
		if(recvBuff[0] == LTU_CMD_HEAD) {
			...
			if(nRet != -1) {
				...
				epicsPrintf("TC %d, Data Recv Count : %d\n", tcmds->card, ltu_head.nDataCnt);

				if(ltu_head.nDataCnt == 10 && tcmds->status == 1) {
					epicsPrintf("--- TC %d, data file save, shot : %d, count : %d ---\n", tcmds->card, tcmds->shot, count);
