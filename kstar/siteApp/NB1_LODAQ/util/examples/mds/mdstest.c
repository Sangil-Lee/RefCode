...
char	MDS_TREE_NAME[256]	= "my_tree";
char	MDS_TAG_NAME[256]	= ":SIGNAL1";
int		MDS_SHOT_NUM		= -1;
int 	MDS_SIZE 			= 1000;
...
int writeExample()
{
	...
	int socket; /* Will contain the handle to the remote
				   mdsip server */
	float data[MDS_SIZE]; /* Array of floats used for signal */
	float timebase[MDS_SIZE]; /* Array of floats used for timebase */
	...
	for(i = 0; i < MDS_SIZE; i++)
	{
		data[i] = sin(i / 300.) * cos(i / 20.);
		timebase[i] = i/MDS_SIZE; /*Build a signal which lasts 1 sec */
	}

	/* Build data descriptor */
	len = MDS_SIZE; /* MDS_SIZE samples */ 
	...
	/* Write The signal */
	status = MdsPut(MDS_TAG_NAME, "BUILD_SIGNAL($1,,$2)", &dataDesc, &timeDesc,
			&null);
	if ( !statusOk(status) )
	{
		/* error */
	...
	/* Close tree */
	status = MdsClose(MDS_TREE_NAME, &shot);
	if ( !statusOk(status) )
	{
