...
void make_local_file(ST_STD_device *pSTDdev)
{
	...
	int chanNum;
	char mkdirbuf[256];
	sprintf (mkdirbuf, "%s/%d", STR_LOCAL_DATA_DIR, (int)pSTDdev->ST_Base_fetch.shotNumber);
	mkdir (mkdirbuf, 0755);
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
        chanNum = ellCount(pSTDdev->pList_Channel);
		...
		for(i=0;i<chanNum;i++) {
			if (NULL != (pSTDCh = get_STCh_from_STDev_chID (pSTDdev,i))) {
				...
				if (CH_USED == pNIDAQmxAICh->used) {
					sprintf (pNIDAQmxAICh->path_name,"%s%d/%s",
							STR_LOCAL_DATA_DIR, (int)pSTDdev->ST_Base_fetch.shotNumber, pNIDAQmxAICh->file_name);
...
int createLocalMdsTree (ST_STD_device *pSTDdev)
{
	...
	return (WR_ERROR);
#else
	// local tree is created by DAQ

	kLog (K_MON, "[createLocalMdsTree] %s : local tree is created by IOC\n", pSTDdev->taskName);
	...
	if (0 != pNIDAQmx->taskHandle) {
		...
		if(0 == strcmp(pNIDAQmx->sigType, "ai")){
			...
			if((pNIDAQmx->smp_mode_select == 1) && (pNIDAQmx->protect_stop_flag == 1)){
				...
				DBproc_put(PV_SXR_AO_OUTPUT_VALUE_CH0_STR, "0");
				DBproc_put(PV_SXR_AO_OUTPUT_VALUE_CH1_STR, "0");
				DBproc_put(PV_SXR_AO_OUTPUT_VALUE_CH2_STR, "0");
				DBproc_put(PV_SXR_AO_OUTPUT_VALUE_PWR_CH0_STR, "0");  // FLNK to the output value ch0 and ch1 about "zero" value 
				epicsThreadSleep (0.1);
			} else if((pNIDAQmx->smp_mode_select == 0)){
				DAQmxTaskControl(pNIDAQmx->taskHandle, DAQmx_Val_Task_Abort);
				NIERROR(DAQmxStopTask(pNIDAQmx->taskHandle));
				...
				for (i=0;i<chanNum;i++) {
					if (NULL != (pSTDCh = get_STCh_from_STDev_chID (pSTDdev,i))) {
						...
						if(pNIDAQmxAICh->inputType == DENSITY_CALC) {  /* init density data channel */
							...
							//remove density	pNIDAQmxAICh->calcDensity[j] = 0;	
							}
							scanIoRequest(pSTDdev->ioScanPvt_userCall);
						}
					}
...
//			pNIDAQmxAICh = (ST_NIDAQmx_AIchannel*) ellNext(&pNIDAQmxAICh->node);
	...
	if (TRUE == isSamplingStop (pSTDdev)) {
		...
		stopDevice (pSTDdev); //to used ao stop
		//stopDeviceAll();
	} 
	return 0;
}
...
int create_NIDAQmx_taskConfig(ST_STD_device *pSTDdev)
{
	...
	if(!pNIDAQmx){
		kLog (K_TRACE, "[create_NIDAQmx_taskConfig- calloc error] ...\n");
		return WR_ERROR;
	}
	...
	if (NULL == pNIDAQmx->data_buf) {
		free (pNIDAQmx);
		pNIDAQmx = NULL;
		kLog (K_TRACE, "[create_NIDAQmx_taskConfig- data_buf error] ...\n");
		return WR_ERROR;
	}
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	...
	pNIDAQmx->terminal_config		= DAQmx_Val_Diff;	// DAQmx_Val_Diff(6133,6259), DAQmx_Val_NRSE NI-6254&NI6220(DAQmx_Val_RSE)
	...
	for (i=0;i<chanNum;i++) {
		if (NULL != (pSTDCh = get_STCh_from_STDev_chID (pSTDdev,i))) {
			...
			if(0 == strcmp(pNIDAQmx->sigType, "ai")){
				...
				sprintf (pNIDAQmxAICh->file_name, "B%d_CH%02d", pSTDdev->BoardID, ((pSTDdev->BoardID * 8) + (i+1)));
