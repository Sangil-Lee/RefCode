...
static void set_cernoxCurrent(drvPXISCXI_taskConfig *ptaskConfig, float64 current);
...
static const iocshArg createPXIHallChannelArg2 = {"min value", iocshArgDouble};
static const iocshArg createPXIHallChannelArg3 = {"max value", iocshArgDouble};
static const iocshArg createPXIHallChannelArg4 = {"gain value", iocshArgDouble};
static const iocshArg* const createPXIHallChannelArgs[] = { &createPXIHallChannelArg0,
	                                                    &createPXIHallChannelArg1,
	                                                    &createPXIHallChannelArg2,
	                                                    &createPXIHallChannelArg3,
	                                                    &createPXIHallChannelArg4};
...
static void set_cernoxCurrent(drvPXISCXI_taskConfig *ptaskConfig, float64 current)
{
	...
	while(pchannelConfig) {
		if(pchannelConfig->channelType == CERNOX_CHANNEL) {
			...
			pcernoxChannelConfig->excitationCurrent = current;
...
static void set_cernoxChanGain(drvPXISCXI_taskConfig *ptaskConfig, float64 gain)
{
	...
	while(pchannelConfig) {
		if(pchannelConfig->channelType == CERNOX_CHANNEL) {
			pcernoxChannelConfig = (drvPXISCXI_cernoxChannelConfig*) pchannelConfig;
/* 2012.4.23 - Cernox Type change CU -> AA, It is overRange(+/-5V) when temp is 5.5K with 100gain. To avoide overRange cernox type AA have to set gain 50.  */
			if(gain <= 100 && pcernoxChannelConfig->gainException == 50){
					pcernoxChannelConfig->gain = 50;
			} else pcernoxChannelConfig->gain = gain;
			DAQmxErrChk(DAQmxSetAIGain(ptaskConfig->taskHandle,
						   pcernoxChannelConfig->chanName,
						   pcernoxChannelConfig->gain));
			DAQmxErrChk(DAQmxGetAIGain(ptaskConfig->taskHandle,
				...
				case CERNOX_CHANNEL:
					...
					if(level>3) {
						drvPXISCXI_cernoxChannelConfig *pcernoxChannelConfig
					        ...
					        epicsPrintf("\n\t minRange: %f, maxRange: %f, gainException:%d, excitationCurrent: %f",
							     pcernoxChannelConfig->minRange,
							     pcernoxChannelConfig->maxRange,
						    	     pcernoxChannelConfig->gainException,
...
/*  Epics Sleep Function is defending Create Before complate the pcontrolThreadConfig  TG.Lee 20090708      */	
	while(!ptaskConfig->pcontrolThreadConfig) epicsThreadSleep(.1);
/*  And Then pcontrolThreadConfig assign */
...
/* 2012.4.23 - Cernox Type change CU -> AA, It is overRange(+/-5V) when temp is 5.5K with 100gain. To avoide overRange cernox type AA have to set gain 50.  */
	if(args[2].ival){
		  pcernoxChannelConfig->gainException = args[2].ival;
	} else pcernoxChannelConfig->gainException = 0;
...
/*	phallChannelConfig->gain = INIT_GAIN_HALL;  */
	...
	phallChannelConfig->minRange = args[2].dval;
	phallChannelConfig->maxRange = args[3].dval;
	phallChannelConfig->gain = args[4].dval;
...
{
	set_cernoxCurrent(ptaskConfig, current);
	return;
}
