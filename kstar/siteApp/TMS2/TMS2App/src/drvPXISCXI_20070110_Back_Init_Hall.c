...
static const iocshArg createPXIStrainChannelArg2 = {"gauge factor", iocshArgDouble};
static const iocshArg createPXIStrainChannelArg3 = {"poisson rate", iocshArgDouble};
...
static void set_samplingRateALLTasks(float64 smaplingRate)
{
	...
	while(ptaskConfig) {
		...
		ptaskConfig = (drvPXISCXIConfig*) ellNext(&ptaskConfig->node);
	}
}
		...
		if(level>2) {
			switch(pchannelConfig->channelType) {
				case CERNOX_CHANNEL:
					...
					break;
				case HALL_CHANNEL:
					epicsPrintf("\tHall Channel, status:0x%x, index: %d",
						    pchannelConfig->channelStatus,
						    pchannelConfig->chanIndex);
					break;
				case STRAIN_CHANNEL:
					epicsPrintf("\tStrain Channel, status: 0x%x, index: %d",
						    pchannelConfig->channelStatus,
						    pchannelConfig->chanIndex);
					break;
...
static void loadCernoxListCallFunction(const iocshArgBuf *args)
{
	...
	while(pcernoxDataConfig = load_cernoxDataOnebyOne(file)) {
...
static void createPXITaskCallFunction(const iocshArgBuf *args)
{
	...
	if(!ptaskConfig) return;
	else ellAdd(pdrvPXISCXIConfig->ptaskList, &ptaskConfig->node);
...
static void createPXIStrainChannelCallFunction(const iocshArgBuf *args)
{
	...
	pstrainChannelConfig->minRange = INIT_MINRANGE_STRAIN;
	pstrainChannelConfig->maxRange = INIT_MAXRANGE_STRAIN;
	pstrainChannelConfig->bridgeType = INIT_BRIDGETYPE_STRAIN;
	pstrainChannelConfig->excitationSource = INIT_EXCITATIONSOURCE_STRAIN;
	pstrainChannelConfig->excitationVoltage = INIT_EXCITATIONVOLTAGE_STRAIN;

	pstrainChannelConfig->gageFactor = args[2].dval;

	pstrainChannelConfig->initialBridgeVoltage = INIT_INITIALBRIDGEVOLTAGE_STRAIN;
	pstrainChannelConfig->nominalGageResistance = INIT_NOMINALGAGERESISTANCE_STRAIN;

	pstrainChannelConfig->poissonRate = args[3].dval;

	pstrainChannelConfig->leadWireResistance = INIT_LEADWIRERESISTANCE_STRAIN;
...
LOCAL long drvPXISCXI_init_driver(void)
{
	...
	epicsAtExit(clearAllDAQmxTasks, NULL);
