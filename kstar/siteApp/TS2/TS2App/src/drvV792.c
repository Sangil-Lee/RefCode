...
void threadFunc_V792_CatchEnd(void *param)
{
	...
	while(TRUE) 
	{
		...
		printf("%s: send TS2 system stop \n", pSTDdev->taskName);
/*		db_put(SYS_RUN, "0"); */
		db_put(pMaster->strPvNames[NUM_SYS_RUN], "0");
		epicsThreadSleep(1.5);
		printf("%s: Master send TS2 disarming \n", pSTDdev->taskName);
		db_put(pMaster->strPvNames[NUM_SYS_ARMING], "0");
...
/*		printf("%s: send TSSC stop pulse.\n", pSTDdev->taskName);
		db_put("TS2_SOFT_CALL_TSSC_STOP_PULSE", "1");
		epicsThreadSleep(1.);
*/
/*		printf("%s: send TS2 release arming \n", pSTDdev->taskName);
		db_put("TS2_SYS_ARMING", "0");
*/

		db_get("TS2_TSSC_LTU_DG535_us", strVal);
		sscanf(strVal, "%d", &pV792->time_LTU_DG535_us);

		db_get("TS2_TSSC_PULSE_CNT", strVal);
		sscanf(strVal, "%u", &pV792->count_DG535_IN_PULSE);

		db_get("TS2_TSSC_BG_DELAY_us", strVal);
		sscanf(strVal, "%u", &pV792->time_Bg_delay_us);

		drvV792_assign_after_shot_info();
/*
		printf("%s: send TSSC stop pulse reset.\n", pSTDdev->taskName);
		db_put("TS2_SOFT_CALL_TSSC_STOP_PULSE", "0");
*/
...
*/
#if 0
				if( (chId == 4 || chId == 4) && pSTDdev->BoardID == 0)
				{
...
//				pV792->ST_Ch[(data_buff[nval]>>16 ) & 0x1F].rawVal_current = data_buff[nval] & 0xFFF;
				pV792->rawVal_current[(data_buff[nval]>>17 ) & 0xF] = data_buff[nval] & 0xFFF;
				break;
			case 4: /* 100 */
...
/* find header_dreg value. temporary. 2011. 6. 20*/
	...
	sprintf(buf, "%s/%d_b0.04.dat", STR_CHANNEL_DATA_DIR, (int)pSTDdev->ST_Base_fetch.shotNumber); 
	fp = fopen(buf, "rb");
...
int drvV792_get_info_befor_shot()
{
	...
	while(pSTDdev)
	{
		...
		if( db_get("TS2_TS_LTU_P0_SEC0_T0", strBuf) == WR_ERROR ) {
			notify_error(1, "'TS2_TS_LTU_P0_SEC0_T0' get error!\n");
			return WR_ERROR ;
		} sscanf(strBuf, "%lf", &pSTDdev->ST_Base.dT0[0]);

		if( db_get("TS2_TSSC_LASER_CLOCK_hz", strBuf) == WR_ERROR ) {
			notify_error(1, "'TS2_TSSC_LASER_CLOCK_hz' get error!\n");
			return WR_ERROR ;
