...
/* Error framSize is not right pixel   image_length = atu32_frameSize * atu32_noSubImages;     5120  = SubImage * 1024(frameSize) */
image_length = atu32_right * atu32_noSubImages;    /* 5120  = SubImage * 1024(frameSize) */
/* Error framSize is not right pixel   total_length = atu32_noFrames * atu32_frameSize * atu32_noSubImages;   */
total_length = atu32_noFrames * atu32_right * atu32_noSubImages;
...
right_pixel = atu32_right;                         /* pixel number : 1024   int => unsigned long int,  atu32_frameSize is 5120.*/
  ...
  for(i=(int)atu32_frameSize; i >= 1; i--) {
      ...
      /* This function is used to retrieve wavelength in each frame in the SIF file.  */
       if(atu32_ret != ATSIF_SUCCESS) {
          if(i<10){
                printf("Could not Get ATSIF_GetPixelCalibration. Error 1: %u\n", atu32_ret);
                }
       }else {
         if(i<10){
 ...
 if(x_cal1 < 0) {
      ...
      /* This function is used to retrieve wavelength in each frame in the SIF file.  */
       if(atu32_ret != ATSIF_SUCCESS) {
            if(i<10){
                printf("Could not Get ATSIF_GetPixelCalibration. Error 2: %u\n", atu32_ret);
                }
       }else {
          if(i<10){
              printf("Wavelength pixel:%d , X: %f   ", i, calibValue);
...
/*float timeCount=0.;   imageNumber * right_pixel  */
	...
	for(i=0; i<10; i++){
	  ROIinfoblkTemp[i].startx = ROIinfoblk[i].startx;
	  ROIinfoblkTemp[i].endx = ROIinfoblk[i].endx;
	  ROIinfoblkTemp[i].groupx = ROIinfoblk[i].groupx;
	  ROIinfoblkTemp[i].starty = ROIinfoblk[i].starty;
	  ROIinfoblkTemp[i].endy = ROIinfoblk[i].endy;
	  ROIinfoblkTemp[i].groupy = ROIinfoblk[i].groupy;
/*	  
...
*/
	...
	image_top = ROIinfoblkTemp[0].endy;              /* Image Top ex.255  */
	image_bottom = ROIinfoblkTemp[0].starty;         /*Image Bottom ex.1  */
	printf("****************leesi++ Number_subImage(%d), right_pixel(%d)\n", number_subImage, right_pixel);
	...
	for (i=total_length -10; i<total_length; i++){
			printf("%d - %hu, ", i, image_buffer1[i]);
	}
#endif
	...
	for (m=0,k=0; m<number_subImage; m++) 
	{
		for (jum=0,j=0; j<imageNumber; j++)
		{
			...
			if(imageNumber>1)
			{
				jum=(j+1)*image_length + (m*right_pixel);  /* right_pixel = 1024, image_length(number_subImage * right_pixel) = 5120 */
				k= jum;
			}
		}
		k = (m+1)*right_pixel ;
		#if 0 
	  for (i=0; i<10; i++){
			printf("%d - %hu, ", i, dataArraySpe[i]);
	  }
	  for (i=10; i<timeCount; i++){
			printf("%d - %hu, ", i, dataArraySpe[i]);
	  }
    #endif
    fprintf(stderr,"Check Data Put shot: %d. m : %d. \n",shotNumber, m);
		fprintf(stdout, "DAQ WAVEMode OK : %s \n",MODEWAVE);
    status  = MdsPut(speChannelTagName[m], "BUILD_SIGNAL($1,,$2)", &dataSpeDesc, &timeSpeDesc, &null); /*  Data Put 2D and Time TEST ING */
			...
			if(m<10){
				sprintf(buf_spe, "FS_FLOAT(%d)", ROIinfoblkTemp[m].starty);
				...
				sprintf(buf_spe, "FS_FLOAT(%d)", ROIinfoblkTemp[m].startx);
				...
				sprintf(buf_spe, "FS_FLOAT(%d)", ROIinfoblkTemp[m].endx);
				...
				sprintf(buf_spe, "FS_FLOAT(%d)", ROIinfoblkTemp[m].endy);
