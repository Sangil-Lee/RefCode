...
char speChannelWaveName[5][21] = {"\\SURVEY_WAVE:FOO", "\\SURVEY_WAVE:X_CAL0", "\\SURVEY_WAVE:X_CAL1", "\\SURVEY_WAVE:X_CAL2", "\\SURVEY_WAVE:X_CAL3"};
  ...
  float x_cal0_spe;
	...
	int number_subImage;                   /* image count, subImage = 5  */
	int right_pixel;                         /* pixel number : 1024   int => unsigned long int  */
	unsigned int	image_length;                  /* 5120  = SubImage * 1024(frameSize) */
...
/* image_length = xDim * yDim;       Input Hard write.           5120  = SubImage(5) * 1024(frameSize) for step. */
image_length = 1024 * 1;
xDim = 1024;
yDim = 1;

	printf("image_length *********************************** here : %u \n",image_length);

imageNumber = (long)numFrames;                    /* imageNumber,    image count at time 30720/5120 = 6  int => unsigned long int  */
number_subImage = (int)yDim;                     /* image count, subImage = 5  */
right_pixel = (int)xDim;                         /* pixel number : 1024   int => unsigned long int  */
  ...
  printf("Check This ######################## 0 \n");
	timeCount = (float)imageNumber*right_pixel;
  total_length =(unsigned int)numFrames * xDim * yDim; 
...
#if 0
   	kkk=1;
	  dataSpeDesc = descr(&dtype_float, dataArraySpe, &right_pixel, &imageNumber, &kkk, &null);  /* We used this desc - descr(TYPE, Data3D, nx,ny,nz,0) ?? */
...
/*				printf("dataArraySpe Put : %f,   image_buffer Data : %hu, k = %uli \n", dataArraySpe[j*right_pixel + i],image_buffer1[k],k);  */
     ...
     printf("##################################################### 6 \n");
#if 0
        printf("dataArraySpe Put. \n");
        for(i=0; i<imageNumber*right_pixel; i++){
         printf("%f,  ", dataArraySpe[i]);
        }
#endif
#if 0
        printf("timeArray Put. \n");
        for(i=0; i<imageNumber; i++){
         printf("%f,  ", timeArray[i]);
        }
