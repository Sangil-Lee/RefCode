...
int stopDevice (ST_STD_device *pSTDdev)
{
	...
	if (0 != pNIDAQ->taskHandle) {
		...
		DAQmxTaskControl(pNIDAQ->taskHandle, DAQmx_Val_Task_Abort);
		NIERROR(DAQmxStopTask(pNIDAQ->taskHandle));
...
void func_NIDAQ_PRE_SEQSTART(void *pArg, double arg1, double arg2)
{
	...
	if (1 == (int)arg1) {
		if (TRUE == isRemoteRunning (pSTDdev)) {
			// TGL remote mode and after shot sequence terminated. we doing trend run mode setting, arming, running until next shot seqence start
			//run stop, arming stop, trend mode disable
			if (OPMODE_REMOTE == pSTDdev->ST_Base.opMode) {
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
	...
	kLog (K_MON, "[EveryNCallback] %s : type(%d) nSamples(%d) rate(%.f) status(%s) total(%d) mode(%d) shot(%lu/%lu)\n",
			pSTDdev->taskName, everyNsamplesEventType, nSamples, pNIDAQ->sample_rate,
...
*/
	...
	// converts scaling eu value to raw value of 14bits resolution
	 for (i=0;i<totSampsAllChan;i++) {
		 pNIDAQ->readArray[i] = GET_RAW18_VALUE(pNIDAQ->fScaleReadArray[i]);
  ...
  /* remove ... because I used not offset value */
	if (TRUE == isCalcRunMode ()) {
		if (0 >= pNIDAQ->totalRead) {
			// save the first offset to compensate
			for(i = 0; i<chanNum; i++) {
				pNIDAQ->ST_Ch[i].offset	= pNIDAQ->readArray[i];

				kLog (K_DEL, "[EveryNCallback] %s : ch(%d) offset(%d) eu(%.5f)\n", pSTDdev->taskName, i,
						pNIDAQ->readArray[i], pNIDAQ->fScaleReadArray[i]);
			}
		}
	...
	while (TRUE) {
		...
		if (pSTDdev->StatusDev & TASK_STANDBY) {
			...
			/*	processAutoTrendRun (pSTDdev);   */
				...
				NIERROR(DAQmxCreateTask("",&pNIDAQ->taskHandle));
				...
				for (i=0;i<chanNum;i++) {
					if(pNIDAQ->ST_Ch[i].inputType == STRAIN_DATA){
						...
						NIERROR ( DAQmxCreateAIStrainGageChan ( pNIDAQ->taskHandle,
									pNIDAQ->ST_Ch[i].physical_channelSCXI,
									NULL,
									pNIDAQ->minVal,   //Min Range 
									pNIDAQ->maxVal,   //Max Range
						...
						kLog (K_INFO, "[NIDAQ_DAQ] AI DATA in DAQmxCreateAIVoltageChan : ch(%d), physical_channel(%s) minVal(%.f) maxVal(%f)\n",
					...
					kLog (K_INFO, "[NIDAQ_DAQ] DAQmxCreateAIVoltageChan : %s\n", pNIDAQ->ST_Ch[i].physical_channel);
				}
				...
				if(pNIDAQ->ST_Ch[i].inputType != STRAIN_DATA){
					/* SCXI type is Backplan clock used by 333kHz. and it is depend on the number of channels  */
        		       		NIERROR(DAQmxGetSampClkMaxRate(pNIDAQ->taskHandle, &(pNIDAQ->sample_rateLimit)));
 				}
...
*/
					...
					kLog (K_INFO, "[NIDAQ_DAQ] %s : waiting trigger %s\n",
				...
				NIERROR(DAQmxStartTask(pNIDAQ->taskHandle));
				...
				kLog (K_INFO, "[threadFunc_NIDAQ_DAQ] before DAQmxClearTask for %s\n", pSTDdev->taskName);
				NIERROR(DAQmxClearTask(pNIDAQ->taskHandle));
				kLog (K_INFO, "[threadFunc_NIDAQ_DAQ] after  DAQmxClearTask for %s\n", pSTDdev->taskName);
				pNIDAQ->taskHandle = 0;
...
int create_NIDAQ_taskConfig(ST_STD_device *pSTDdev)
{
	...
	kLog (K_TRACE, "[create_NIDAQ_taskConfig] task(%s) dev(%s) trig(%s) sigType(%s)\n",
...
/* TGLee change from INIT_SAMPLE_RATE to chanNum(ellCount) */
        chanNum = ellCount(pSTDdev->pList_Channel);
	kLog (K_TRACE, "[create_NIDAQ_taskConfig] task(%s) dev(%s) trig(%s) sigType(%s) channelCounts(%d)\n",
	...
	kLog (K_INFO, "[create_NIDAQ_taskConfig] dev(%s) triggerSource(%s) checkTriggerSource(%s) \n",
					pSTDdev->strArg0,pSTDdev->strArg1,pNIDAQ->triggerSource);

	pNIDAQ->terminal_config			= DAQmx_Val_Diff;	// DAQmx_Val_Diff(6133,), DAQmx_Val_NRSE NI-6254&NI6220(DAQmx_Val_RSE)
	...
	pSTDdev->ST_Func._CREATE_LOCAL_SHOT	= func_NIDAQ_CREATE_LOCAL_SHOT;
...
// In this function, I don't have much time to control sequence, so move this function to snl sequence control by shotTree create event 
			...
			for(i=0;i<chanNum;i++) {
				if(pNIDAQ->ST_Ch[i].inputType == DENSITY_CALC) {
					...
					epicsPrintf(" Ch No:%d, Tag Name : %s, euValue : %f \n",
						i, pNIDAQ->ST_Ch[i].strTagName,pNIDAQ->ST_Ch[i].euValue);
					epicsPrintf("\n");
